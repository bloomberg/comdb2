...
retry_queries:
    ...
    if (!hndl->sb) {
        if (is_rollback) {
            ...
            if (hndl->debug_trace) {
                fprintf(stderr, "td %u %s:%d returning 0 on unconnected rollback\n", 
                        (uint32_t) pthread_self(), __func__, __LINE__);
            }
            PRINT_RETURN(0);
        }
        ...
        if (hndl->sb == NULL) {
            if (hndl->debug_trace) {
                fprintf(stderr, "td %u %s:%d rc=%d goto retry_queries on connect "
                        "failure\n", (uint32_t) pthread_self(), __func__, __LINE__, rc);
            }
            goto retry_queries;
        }
...
read_record:
    ...
    if (rc) {
        ...
        if (hndl->is_hasql || commit_file) {
            ...
            hndl->retry_all = 1;
            if (hndl->debug_trace) {
                fprintf(stderr, "td %u %s:%d goto retry_queries rc=%d, err_val=%d\n", 
    ...
    if (hndl->first_buf != NULL) {
        ...
        if (hndl->debug_trace) {
            fprintf(stderr,
                    "td %u %s:%d: Can't read response from the db\n",
                    (uint32_t)pthread_self(), __func__, __LINE__);
        }
    ...
    if (hndl->firstresponse->response_type == RESPONSE_TYPE__COLUMN_NAMES) {
        ...
        if (is_retryable(hndl, hndl->firstresponse->error_code)) {
            ...
            if (hndl->debug_trace) {
                fprintf(stderr, "td %d %s:%d: goto retry_queries error_code=%d\n",
                        (uint32_t)pthread_self(), __func__, __LINE__, 
                        hndl->firstresponse->error_code);
            }
        ...
        if (hndl->is_hasql && (((is_retryable(hndl, rc) && hndl->snapshot_file) ||
            ...
            if (hndl->debug_trace) {
                fprintf(stderr, "td %d %s:%d: goto retry_queries retry-begin, error_code=%d\n",
                        (uint32_t)pthread_self(), __func__, __LINE__, 
                        hndl->firstresponse->error_code);
            }
                             ...
                             int *num_valid_sameroom_hosts)
{
    ...
    if (hndl->debug_trace)
        fprintf(stderr,
                "td %d %s:%d, cdb2_socket_pool_get fd %d, host '%s'\n",
