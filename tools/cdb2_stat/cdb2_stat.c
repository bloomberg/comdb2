	...
	char *argv[];
{
	...
	char *db, *home, *internal, passwd[1024], *subdb;
	FILE *crypto;
	...
	u_int32_t flags;
{
	...
	dl("Total log file writes due to log sync.\n", (u_long)sp->st_swrites);
	...
	u_int32_t flags;
{
	...
	dl("Clean leaf pages forced from the cache.\n",
	    (u_long)gsp->st_ro_levict);
	dl("Dirty leaf pages forced from the cache.\n",
	    (u_long)gsp->st_rw_levict);
	dl("Dirty pages encountered while forcing clean pages.\n",
	    (u_long)gsp->st_rw_evict_skip);
	...
	dl("The number of pages sync'd using perfect checkpoint\n",
	    (u_long)gsp->st_ckp_pages_sync);
	dl("The number of pages skipped using perfect checkpoint\n",
	    (u_long)gsp->st_ckp_pages_skip);
