...
#define	atomic_compare_exchange(env, p, o, n)	\
	___atomic_compare_exchange((p), (o), (n))
 ...
 */
static inline int ___atomic_compare_exchange(
...
/* AIX compiler uses some gcc builtins */
#define	atomic_inc(env, p)	__atomic_inc(p)
#define	atomic_dec(env, p)	__atomic_dec(p)
#define	atomic_compare_exchange(env, p, o, n)	\
	compare_and_swap((p), (o), (n))
...
static inline int __atomic_inc(db_atomic_t *p)
{
	...
	temp = 1;
	temp = __sync_fetch_and_add(&p->value, temp);
	return (temp + 1);
}
...
static inline int ___atomic_compare_exchange(
	db_atomic_t *p, atomic_value_t oldval, atomic_value_t newval)
{
	return __sync_bool_compare_and_swap((int32_t*) p, oldval, newval);
}
