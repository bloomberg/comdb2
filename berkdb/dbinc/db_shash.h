...
#define	HASHLOOKUP(begin, ndx, type, field, elt, res, cmp) do {		\
	SH_TAILQ_HEAD(, type) *__bucket;				\
	__bucket = (void *)&begin[ndx];					\
 ...
 */
#define	HASHREMOVE_EL(begin, ndx, type, field, obj) do {		\
	SH_TAILQ_HEAD(, type) *__bucket;				\
	__bucket = (void *)&begin[ndx];					\
...
#define __db_hashinit(begin, nelements) do {				\
	u_int32_t i;							\
	for (i = 0; i < nelements; ++i)					\
		SH_TAILQ_INIT(&begin[i]);				\
} while (0)
