...
#define	ITEM_END	2
		 ...
		 * except in recno.
		 */
		if (B_DISSET(bk) && TYPE(h) != P_LRECNO) {
			isbad = 1;
	...
	"Page %lu: duplicate page referenced by recno page at item %lu",
		...
		case B_OVERFLOW:
			...
			child.pgno = bo->pgno;
			child.type = (B_TYPE(bk) == B_OVERFLOW ?
		...
		default:
			...
			EPRINT((dbenv, "Page %lu: item %lu of invalid type %lu",
			    (u_long)pgno, (u_long)i, (u_long)bk->type));
			break;
		}
	}

	/* 
	 * pfx compressed pages are not aligned. They also appear to
	 * overlap as the prefix is stored at the end of the page.
	 * Skip check for pfx compressed pages.
	 */
	if (IS_PREFIX(h) == 0) {
		/*
	...
	u_int32_t flags;
{
	...
	for (i = 0;; i += O_INDX) {
		...
		if (ret == 0) {
			 ...
			 */
			bk = GET_BKEYDATA(dbp, h, i);
			if (!LF_ISSET(DB_AGGRESSIVE) && B_DISSET(bk))
				continue;
