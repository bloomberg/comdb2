 ...
 * when gbl_lock_instrumentation is on.  "repl" is the replication apply thread,
 * identified by gbl_rep_lockid rather than inferred; "other" is everything left
 * over -- verify, analyze, schema change, internal threads.
 */
struct lock_role_stats {
	...
	u_int64_t repl_wait_us;
	u_int64_t repl_waits;
 ...
 * where the tagged fields are only present when their tag is set.
 */
static inline u_int32_t
...
/* 'prefix' is __rectype_prefix_len() of the record's raw (tagged) rectype. */
