	...
	if (rectype == DB___txn_regop_rowlocks) {
		...
		*ltrans = lt;
		...
		if (lflags & DB_TXN_SCHEMA_LOCK) {
			if (lockid == 0)
				get_schema_lock = 1;
			/* If lockid isn't 0, then this is parallel rep forced serial: 
...
err:
	...
	if (release_schema_lock ||
		(ret == 0 && txn_rl_args && txn_rl_args->lflags & DB_TXN_SCHEMA_LOCK)) {
		unlock_schema_lk();
	}
