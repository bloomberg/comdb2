...
int normalize_rectype(u_int32_t *rectype) {
	...
	// present.  Keeps the ufid tag (callers strip it) and DB_debug_FLAG.

	u_int32_t base, tags = __rectype_tags(*rectype, &base);

	*rectype = (*rectype & DB_debug_FLAG) | (base +
...
matchable_log_type(DB_ENV *dbenv, int rectype)
{
	...
	int ret;
	/* XXX do not ever match DB___txn_ckp_recovery XXX */
...
static inline void repdb_enqueue(REP_CONTROL *rp, DBT *rec, int decoupled)
{
	...
	int repdb_maxlog = gbl_inmem_repdb_maxlog, most_recent = gbl_inmem_repdb_most_recent; 
	Pthread_mutex_lock(&inmem_repdb_lk);

	if (repdb_maxlog <= 0)
		repdb_maxlog = 1000;

    /* If we are at the max, and this is earlier than the top, just drop */
	if (listc_size(&repdb_queue) >= repdb_maxlog) {
		if (most_recent) {
			struct repdb_rec *top = LISTC_TOP(&repdb_queue);
			if (log_compare(&top->repctl->lsn, &rp->lsn) >= 0) {
				if (gbl_debug_inmem_repdb) {
					logmsg(LOGMSG_USER, "Not enqueing LSN %u:%u, under min and top is %u:%u\n", rp->lsn.file, rp->lsn.offset, top->repctl->lsn.file, top->repctl->lsn.offset);
				}
	...
	/* Limit size of list to tunable, removing lowest LSNS */
	while (listc_size(&repdb_queue) > repdb_maxlog) {
		r = most_recent ? listc_rtl(&repdb_queue) : listc_rbl(&repdb_queue);
		gbl_inmem_repdb_memory -= (r->size + sizeof(*r->repctl) + sizeof(*r));
...
gap_check:		use_range = 0;
		...
		if (gbl_disable_repdb) {
			...
			DB_LSN oldwaiting = lp->waiting_lsn;
			Pthread_mutex_lock(&inmem_repdb_lk);
			assert (listc_size(&repdb_queue) > 0);
			struct repdb_rec *top = LISTC_TOP(&repdb_queue);
			lp->waiting_lsn = top->repctl->lsn;
			Pthread_mutex_unlock(&inmem_repdb_lk);
			if (log_compare(&oldwaiting, &lp->waiting_lsn) && dbenv->rep_signal_logfill) {
				dbenv->rep_signal_logfill(dbenv, &lp->waiting_lsn);
			}
			ret = 0;
		} else {
		...
		if (IS_ZERO_LSN(lp->waiting_lsn) ||
			...
			if (!gbl_disable_repdb && !gbl_inmem_repdb) {
	...
	if (get_locks_and_ack) {
		...
		ret = context
			...
			: __lock_get_list_context(dbenv, lockid, flags,
				DB_LOCK_WRITE, copy_compare ? &lock_dbt_copy : lock_dbt, &context, &(rctl->lsn),
				&pglogs, &keycnt);
		...
		if (rectype == DB___txn_child) {
			...
			*lsnp = argp->prev_lsn;
			...
			if (gbl_ufid_add_on_collect && !DB_RECTYPE_IS_LOGICAL(rectype) && DB_RECTYPE_HAS_UFID(rectype)) {
				...
				if ((int)ufid_for_recovery_record(dbenv, NULL, rectype, ufid, &data, __rectype_prefix_len(raw_rectype))) {
	...
	int endianize;
{
	...
	ret = __lock_get_list_context(dbenv, lockid, flags,
			DB_LOCK_WRITE, copy_compare ? &lock_dbt_copy : lock_dbt, &context, &lsn, &pglogs, &keycnt);
	if (copy_compare) {
		if (memcmp(lock_dbt_copy.data, lock_dbt->data, lock_dbt->size) != 0) {
			logmsg(LOGMSG_ERROR, "%s:%d lock_get_list modified the lock_dbt\n", __func__, __LINE__);
			abort();
