	...
	u_int32_t *gen;
{
	...
	*gen = rep->committed_gen;
	...
	int vote_on_ckp = gbl_recovery_ckp;
	int regop_cnt = 0;
	...
	while (ret == 0 && rectype != DB___txn_regop_gen && rectype !=
		...
		if (rectype == DB___txn_regop && !vote_on_ckp) {
			...
			/* Tolerate txn_regop to a point (unfortunately).  The code has
			 * changed to intentionally write txn_regop rather than regop_gen
			 * during full recovery.  This goes away when recovery-ckp is
			 * enabled, because then we guarantee that a normal checkpoint
			 * is only ever emitted by the cluster-master */
