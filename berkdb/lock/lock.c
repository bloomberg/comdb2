...
/* Published by rep_record.c while the replication thread is applying, 0 between.
 * Used to attribute its lock waits instead of lumping them into "other". */
	...
	case SECOND:
		...
		if (gbl_bb_berkdb_enable_thread_stats) {
			...
			if (gbl_lock_instrumentation) {
				...
				} else if (gbl_rep_lockid &&
				    sh_locker->id == gbl_rep_lockid) {
					/* The replication apply thread, named
					 * rather than guessed: rep_record.c
					 * publishes its lockid while it applies
					 * and zeroes it in between.  On a
					 * cluster this is where write-side
					 * contention shows up, since SQL does
					 * not run on the master and the master's
					 * own write txns never block. */
					lock_role_stats.repl_wait_us += d;
					lock_role_stats.repl_waits++;
				} else {
