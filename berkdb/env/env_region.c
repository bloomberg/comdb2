...
static void __db_heap_destroy __P((DB_ENV *, void *));
...
extern char *__dbenv_regiontype(int type);
...
loop:	renv = NULL;
	...
	if (F_ISSET(dbenv, DB_ENV_CREATE)) {
		...
		if ((ret = __os_open(dbenv, infop->name, flags,
	...
	flags |= DB_OSO_REGION;
	if (F_ISSET(dbenv, DB_ENV_DIRECT_DB)) {
		logmsg(LOGMSG_INFO, "DB_ENV_DIRECT_DB is set\n");
		flags |= DB_OSO_DIRECT;
	}
	if (F_ISSET(dbenv, DB_ENV_OSYNC)) {
		logmsg(LOGMSG_INFO, "DB_ENV_OSYNC is set\n");
		flags |= DB_OSO_OSYNC;
	}
...
creation:
	 ...
	 */
	memset(&tregion, 0, sizeof(tregion));
	tregion.size = (roff_t) ((50 + dbenv->mp_ncache) * sizeof(REGION) +
	...
	infop->primary = R_ADDR(infop, 0);
	infop->addr = (u_int8_t *)infop->addr + sizeof(REGENV);
	__db_shalloc_init(dbenv, "env", infop->addr,
	    tregion.size - sizeof(REGENV), &((REGENV *)infop->primary)->mutex);
...
retry:	/* Close any open file handle. */
	...
	if (infop->addr != NULL) {
		...
		/* Before we "correct" addr, discard __heap structure */
...
static void
__db_heap_destroy(dbenv, addr)
	DB_ENV *dbenv;
	void *addr;
{
	HEAP *h;
	if (__gbl_use_malloc_for_regions) {
		(void)memcpy(&h, addr, sizeof(HEAP*));
		(void)listc_rfl(&dbenv->regions, h);
		__os_free(dbenv, h->description);
		(void)comdb2ma_destroy(h->msp);
		__os_free(dbenv, h);
	}
}
