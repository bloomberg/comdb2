...
int
__dbenv_setattr(DB_ENV *dbenv, char *attr, char *val, int ival)
{
#undef BERK_DEF_ATTR
...
int __dbenv_getattr
__P((DB_ENV *dbenv, char *attr, char **val, int *ival))
{
#undef BERK_DEF_ATTR
#define BERK_DEF_ATTR(option, description, type, default_value) \
    else if (strcmp(#option, attr) == 0) {                      \
...
int __dbenv_dumpattrs
__P((DB_ENV *dbenv, FILE *out))
{
...
#define BERK_DEF_ATTR(option, description, type, default_value)                         \
    if (type == BERK_ATTR_TYPE_INTEGER || type == BERK_ATTR_TYPE_PERCENT)                                                 \
        logmsgf(LOGMSG_USER, out, "%-60s: %-10d (%s)\n", description, dbenv->attr.option, #option);      \
    else if (type == BERK_ATTR_TYPE_BOOLEAN)                                            \
        logmsgf(LOGMSG_USER, out, "%-60s: %-10s (%s)\n", description, dbenv->attr.option ? "enabled" : "disabled", #option);
#include "dbinc/db_attr.h"

	return 0;
}

void __dbenv_attr_init
__P((DB_ENV *dbenv))
{
