	...
	DB_LOG *dblp;
{
	...
	/* A file holding only its LOGP header (crash mid-rollover) has no
	 * record to anchor on: step back into the previous file.  Runs after
	 * lp->len has consumed logc->c_len, which DB_PREV overwrites. */
	if (!saw_rec) {
		DB_LSN plsn = lsn;
	...
	DB_LSN *lsn, *ckplsn, *trunclsn;
{
	...
	c_len = logc->c_len;
	/* Re-anchor on the last real record at or before the truncation
	 * point; truncating to a LOGP header means stepping back, possibly
