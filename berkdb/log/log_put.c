	...
	void *usr_ptr;
{
    ...
    /* Records patched under the lock (commit context and/or prev_cksum)
     * defer encrypt+checksum to __log_put_next(). */
	...
	u_int32_t flags;
{
	...
	*lsn = lp->lsn;
	...
	u_int32_t cksum_off = prefix - (u_int32_t)sizeof(u_int32_t);
	normalize_rectype(&rectype);
	...
	u_int32_t flags;
{
	...
	if (__txn_force_abort(dbenv,
		...
		/* Re-anchor only if this is still the last record: a master
		 * drops the region lock to send, so more may follow it. */
		if (lp->lsn.file == flush_lsn.file &&
		    lp->lsn.offset - lp->len == flush_lsn.offset)
	...
	DB_LSN *lsnp;
{
	...
	lsn = lp->lsn;
	/* LOGP stays out of the chain (log_size may differ between nodes);
	 * this file's first record chains to the previous file's last. */
