...
static struct sp_source syssps[] = {
    ...
    {"sys.info.cluster", "local function main()\n"
                         "    local schema = {\n"
                         "        { 'string', 'host' },\n"
                         "        { 'int',    'port' },\n"
                         "        { 'int',    'master'},\n"
    ...
    {"sys.info.comdbg_tables", "local function main()\n"
                               "    local schema = {\n"
                               "        { 'string', 'tablename' },\n"
                               "        { 'int',    'dbnum' },\n"
                               "        { 'int',    'lrl'},\n"
    ...
    {"sys.cmd.send", "local function main(cmd)\n"
                     "    local schema = {\n"
                     "        { 'string', 'out' },\n"
                     "    }\n"
                     "    db:num_columns(table.getn(schema))\n"
                     ...
                     "    end\n"
                     "end\n"},
    {"sys.replicate", "local function main(chg) \n"
                      "   local stmt = db:exec('select max(id) as MAX from comdb2_replog')\n"
                      "   if (stmt == nil) then\n"
                      "       stmt = db:exec('create table comdb2_replog{ schema {"
                      "                                    longlong id null = yes"
                      "                                    cstring type[32] "
                      "                                    cstring tbl[32] "
                      "                                    vutf8 new[128] null=yes"
                      "                                    vutf8 old[128] null=yes }}')"
                      "       if(stmt) then\n"
                      "           stmt = db:exec('select max(id) as MAX from comdb2_replog')\n"
                      "       end\n"
                      "   end\n"
                      "   local row  = stmt:fetch()\n"
                      "       local MAX \n"
                      "       if(db:isnull(row.MAX)) then\n"
                      "         MAX = 1\n"
                      "       else\n"
                      "         MAX = row.MAX + 1\n"
                      "       end\n"
                      "   local audit = db:table('comdb2_replog') \n"
                      "   if chg.new ~= nil then\n"
                      ...
                      "end"},
    {"sys.applylocal", "local function getkeyvalues_insert(dbrow)\n"
                       "local new = db:json_to_table(dbrow)\n"
                       "local keys = nil\n"
                       "local values = nil\n"
                       "for id, value in pairs(new) do\n"
                       "  print(id, value)\n"
                       "  if (keys == nil) then\n"
                       "    keys = id\n"
                       "  else\n"
                       ...
                       "return keys, values\n"
                       "end\n"
                       "local function getkeyvalues_delete(dbrow)\n"
                       "local new = db:json_to_table(dbrow)\n"
                       "local where = nil\n"
                       "for id, value in pairs(new) do\n"
                       "  print(id, value)\n"
                       ...
                       "  else\n"
                       "    where = where .. ' and ' .. id .. ' = ' ..  \"'\"..value.. \"'\"\n"
                       "  end\n"
                       "end\n"
                       "return where\n"
                       "end\n"
                       "local function main(dbtype, dbname)\n"
                       "db:setmaxinstructions(1000000000)\n"
                       "db:settimezone('GMT')\n"
                       "local stmt = db:exec('select max(id) as MAX from comdb2_replog')\n"
                       "if (stmt == nil) then\n"
                       "stmt = db:exec('create table comdb2_replog { schema { longlong id null = "
                       "yes cstring type[32] cstring tbl[32] vutf8 new[128] null=yes vutf8 "
                       "old[128] null=yes }}')\n"
                       "if(stmt) then\n"
                       "   stmt = db:exec('select max(id) as MAX from comdb2_replog')\n"
                       "end\n"
                       ...
                       "LOCAL_MAX = row.MAX + 1\n"
                       "end\n"
                       "local query  = 'select max(id) as MAX  from ' .. dbtype .. '_' .. dbname "
                       ".. '.comdb2_replog'\n"
                       "stmt = db:exec(query)\n"
                       "row  = stmt:fetch()\n"
                       "local MAX\n"
                       ...
                       "MAX = row.MAX + 1\n"
                       "end\n"
                       "while (LOCAL_MAX < MAX) do\n"
                       " local query  = 'select * from ' .. dbtype .. '_' .. dbname .. "
                       "'.comdb2_replog where id=' .. LOCAL_MAX\n"
                       "local result = db:exec(query)\n"
                       "local dbrow = result:fetch()\n"
                       "result:close()\n"
                       "db:begin()\n"
                       "local rep_tbl = db:table('comdb2_replog')\n"
                       "local rc = rep_tbl:insert(dbrow)\n"
                       "if not db:isnull(dbrow.old) then\n"
                       "        local where  = getkeyvalues_delete(dbrow.old)\n"
                       "        local query = 'delete from ' .. dbrow.tbl ..  where .. ' limit "
                       "1'\n"
                       "        print(query)\n"
                       "        result = db:exec(query)\n"
                       "        result:close()\n"
                       "end\n"
                       "if not db:isnull(dbrow.new) then\n"
                       "        local keys, values  = getkeyvalues_insert(dbrow.new)\n"
                       "        local query = 'insert into ' .. dbrow.tbl .. '(' .. keys .. ')' "
                       ".. 'values (' .. values ..')'\n"
                       "        print(query)\n"
                       "        result = db:exec(query)\n"
                       "        result:close()\n"
                       ...
                       " return 0, 'success'\n"
                       "end\n"},
    {// to call analyze for a table: cdb2sql adidb local 'exec procedure
     // sys.cmd.analyze("t1")'
     "sys.cmd.analyze", "local function main(tbl, percent)\n"
                        "    local schema = {\n"
                        "        { 'string', 'out' },\n"
                        "    }\n"
                        "    db:num_columns(table.getn(schema))\n"
                        ...
                        "    end\n"
                        "end\n"},
    {// to call verify for a table: cdb2sql adidb local 'exec procedure
     // sys.cmd.verify("t1")'
     "sys.cmd.verify", "local function main(tbl)\n"
