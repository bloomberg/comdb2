...
static struct sp_source syssps[] = {
    ...
    },
    {
        ...
        "local stmt = db:exec('select max(id) as MAX from comdb2_replog')\n"
         "if (stmt == nil) then\n"
        "stmt = db:exec('create table comdb2_replog { schema { longlong id null = yes cstring type[32] cstring tbl[32] vutf8 new[128] null=yes vutf8 old[128] null=yes }}')\n"
        "if(stmt) then\n"
        "   stmt = db:exec('select max(id) as MAX from comdb2_replog')\n"
        "end\n"
        "end\n"
        ...
        "LOCAL_MAX = row.MAX + 1\n"
        "end\n"
        "local query  = 'select max(id) as MAX  from ' .. dbtype .. '_' .. dbname .. '.comdb2_replog'\n"
        "stmt = db:exec(query)\n"
        "row  = stmt:fetch()\n"
        "local MAX\n"
        "if(db:isnull(row.MAX)) then\n"
        ...
        " local query  = 'select * from ' .. dbtype .. '_' .. dbname .. '.comdb2_replog where id=' .. LOCAL_MAX\n"
	...
	"if not db:isnull(dbrow.old) then\n"
        ...
        "        local query = 'delete from ' .. dbrow.tbl ..  where .. ' limit 1'\n"
        "        print(query)\n"
	...
	"if not db:isnull(dbrow.new) then\n"
        ...
        "        local query = 'insert into ' .. dbrow.tbl .. '(' .. keys .. ')' .. 'values (' .. values ..')'\n"
        "        print(query)\n"
        "        result = db:exec(query)\n"
        "        result:close()\n"
	"end\n"
