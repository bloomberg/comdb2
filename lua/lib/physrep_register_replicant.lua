...
local function main(dbname, hostname, lsn, source_dbname, source_hosts)
    ...
    local foundrow = 0
    local testfallback = tunables["testfallback"]

    if testfallback == 0 then
        print("physrep_register_replicant: normal mode")
    else
        print("physrep_register_replicant: testfallback mode")
    end
    ...
    if (rc == 0) then
        ...
        if (foundrow == 0) then
            print("physrep_register_replicant: found no candidates, returning source-db '" .. source_dbname .. "' source-hosts '" .. source_hosts .. "'")
            local sql
            sql = ("SELECT 0, dbname, host FROM comdb2_physreps WHERE dbname = '" .. source_dbname .. "' AND host IN (" .. source_hosts .. ")")
            rs, rc = db:exec(sql)
