...
static int check_proper_debug_log(DB_ENV *dbenv, DB_LOGC *logc, 
        DB_LSN *lsn)
{
    ...
    if(hash_find_readonly(fs->fnames, src) == NULL) {
        ...
        if (stat(src, &sb) == 0) {
            logmsg(LOGMSG_WARN, "MISSING %s from %d:%d-%d:%d\n",
                   src, fs->debug.file, fs->debug.offset,
                   fs->ckp.file, fs->ckp.offset);
...
static int compare_filesets(bdb_state_type *bdb_state,
    file_set_t *fs, DB_LSN *lsn, file_set_t *prev_fs, DB_LSN *prev_lsn)
{
    int rc = 0;

    logmsg(LOGMSG_INFO, "COMPARING fs %d:%d vs older %d:%d\n", 
            lsn->file, lsn->offset, prev_lsn->file, prev_lsn->offset);
