...
int get_dbnum_by_name( bdb_state_type *bdb_state, const char *name)
{
   ...
   for (i = 0; i < bdb_state->parent->numchildren; i++)
      if (strncasecmp(bdb_state->parent->children[i]->name, name, strlen(name)) == 0)
      {
         Pthread_mutex_unlock(&(bdb_state->children_lock));
         return i;
      }
...
/* check if any of the file on disks start with tblname, and have the known name format 
   (depending on file type), and if they are, queue them to the to-be-deleted file list
 */
int bdb_check_files_on_disk( bdb_state_type *bdb_state, const char *tblname, int *bdberr)
{
   ...
   while ((error = readdir_r(dirp, buf, &ent)) == 0 && ent != NULL)
   {
       ...
       * table */
      if (
            (strlen(ent->d_name) > strlen(table_prefix))
         ...
         )
      {
         ...
         {
            uint8_t *p_buf=(uint8_t *)&invers, *p_buf_end=p_buf+sizeof(invers);
            struct bdb_file_version_num_type p_file_version_num_type;
...
/* given an existing table pointed by bdb_state, check the disk for older versions of it
   (i.e. not matching current metadata versioning information), and queue those for deletion
   NOTE: obviously, this is only working for recovering to-be-deleted files during
