...
int bdb_apprec(DB_ENV *dbenv, DBT *log_rec, DB_LSN *lsn, db_recops op)
{
    ...
    if (bdb_state->attr->llog && !gbl_rowlocks && (op == DB_TXN_FORWARD_ROLL || op == DB_TXN_APPLY)) {
        switch (rectype) {
        case DB_llog_scdone:
        case DB_llog_blkseq:
            break;
    ...
    case DB_llog_fingerprint:
        rc = llog_fingerprint_read(dbenv, log_rec->data, &fingerprint);
        if (rc)
            return rc;
        logp = fingerprint;
