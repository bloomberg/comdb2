                               ...
                               void **ppKeyMaxBuf)
{
    ...
    if (bdb_keycontainsgenid(bdb_state, ixnum)) {
        ...
        char *buf = *ppKeyMaxBuf  = malloc(MAXKEYLEN + 1 + sizeof(unsigned long long));
        memcpy(buf, ixdta, bdb_state->ixlen[ixnum]);

        p_dbt_key->data = buf;
        p_dbt_key->size = bdb_state->ixlen[ixnum];

        memcpy(buf + bdb_state->ixlen[ixnum], &tmpgenid, sizeof(unsigned long long));
...
#endif
    assert(rc == 0 || 0 == memcmp(((char *)*ppKeyMaxBuf) + bdb_state->ixlen[ixnum],
