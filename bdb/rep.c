                             ...
                             int comcount, const char *func, uint32_t line)
{
    ...
    if (!bdb_state->attr->enable_seqnum_generations) {
        if (verify_trace && (now = time(NULL)) != lastpr) {
            logmsg(LOGMSG_USER, "%s master-lease return good because "
...
static inline int should_copy_seqnum(bdb_state_type *bdb_state,
                                     seqnum_type *seqnum,
                                     ...
                                     uint8_t is_tcp)
{
    ...
    if (bdb_state->attr->enable_seqnum_generations) {
        ...
        rc = bdb_berkdb_get_attr(bdb_state, "elect_highest_committed_gen", &unused,
                    &elect_highest);
        if (rc) {
            logmsg(LOGMSG_ERROR, "Error getting elect_highest_committed_gen, "
                    "%d\n", rc);
...
int request_copydelay(void *bdb_state_in)
{
    ...
    rc = net_send_flags(bdb_state->repinfo->netinfo,
                        bdb_state->repinfo->master_host,
                        USER_TYPE_COMMITDELAYTIMED, NULL, 0, NET_SEND_NODROP);
    return rc;
}
                                  ...
                                  int dtalen, uint8_t is_tcp)
{
    ...
    case USER_TYPE_COMMITDELAYTIMED:
        ...
        logmsg(LOGMSG_WARN, "--- got commitdelaytimed req from node %s.  timeout=%"PRId64"\n",
                from_node, commit_delay_timeout);
        break;
