                                      ...
                                      int offset, int is_ha_retry)
{
    ...
    if ((shadow_tran->tranclass == TRANCLASS_SNAPISOL ||
        ...
        if (epoch || file || offset || gbl_rowlocks) {
            backfill_required = 1;
        }
    }

    // We should have requested a durable lsn already.
    if ((shadow_tran->tranclass == TRANCLASS_SNAPISOL ||
         shadow_tran->tranclass == TRANCLASS_SERIALIZABLE) &&
        durable_lsns && file == 0) {
        abort();
    ...
    if (file) {
        ...
        if (log_compare(&my_lsn, &arg_lsn) < 0)
        {
            if (gbl_extended_sql_debug_trace) {
                logmsg(LOGMSG_USER, "%s line %d: returning not-durable,"
                        " i am behind, requested-lsn=[%d][%d], my_lsn=[%d][%d]\n", 
                        __func__, __LINE__, file, offset, my_lsn.file, 
                        my_lsn.offset);
            }
