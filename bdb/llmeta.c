...
int bdb_get_file_version_data( bdb_state_type *bdb_state, tran_type *tran,
        int dtanum, unsigned long long *version_num, int *bdberr)
{
    return bdb_get_file_version(tran, bdb_state->name, LLMETA_FVER_FILE_TYPE_DTA,
            dtanum, version_num, bdberr);
}

int bdb_get_file_version_index( bdb_state_type *bdb_state, tran_type *tran,
        int ixnum, unsigned long long *version_num, int *bdberr)
{
...
int bdb_get_file_version_table( bdb_state_type *bdb_state, tran_type *tran,
    unsigned long long *version_num, int *bdberr)
{
    return bdb_get_file_version( tran, (bdb_state)->name, LLMETA_FVER_FILE_TYPE_TBL,
            0, version_num, bdberr);
}
...
{
    return bdb_get_file_version( tran, name, LLMETA_FVER_FILE_TYPE_DTA,
            file_num, version_num, bdberr);
}

int bdb_get_file_version_index_by_name(tran_type *tran, const char *name,
        int file_num, unsigned long long *version_num, int *bdberr )
{
    return bdb_get_file_version( tran, name, LLMETA_FVER_FILE_TYPE_IX,
            file_num, version_num, bdberr);
}
        ...
        int *bdberr)
{
    ...
    rc = bdb_lite_fetch_partial(llmeta_bdb_state, key, klen,
            out, &fnd, bdberr);
    while (rc == 0 && fnd == 1) {
        ...
        if((irc = (*func)(bdb_state, arg, out)) != 0 ) break;

        rc = bdb_lite_fetch_keys_fwd(llmeta_bdb_state, out, nxt, 1, &fnd, bdberr);
        memcpy(out, nxt, sizeof(out));
    }
    return irc?irc:rc;
}
...
#else
    rc = ((int(*)(bdb_state_type*,const char*,int*))arg)(bdb_state, tblname, &bdberr);
#endif
    ...
    int (*func)(bdb_state_type *bdb_state,const char* tblname, int *bdberr), int *bdberr)
{
    ...
    return bdb_process_each_entry(bdb_state, &key, sizeof(key.file_type),
            (int(*)(bdb_state_type*,void*,void*))table_version_callback,
            (void*)func,
            bdberr);
}

static int table_file_callback(bdb_state_type *bdb_state, unsigned long long *file_version,
