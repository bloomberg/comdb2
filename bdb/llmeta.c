...
{
    int rc;
    rc = bdb_get_file_version(tran, bdb_state->name,
                              LLMETA_FVER_FILE_TYPE_DTA, dtanum, version_num,
                              bdberr);
...
int bdb_process_each_table_version_entry_resumable(
        bdb_state_type *bdb_state,
        int (*func)(bdb_state_type *bdb_state, const char *tblname, int *bdberr),
        void **resume, int *bdberr)
{
    ...
    return bdb_process_each_entry_resumable(bdb_state, NULL, &key, sizeof(key.file_type),
                                  (int (*)(bdb_state_type *, tran_type *, void *, void *))table_version_callback,
                                  (void *)func, resume, bdberr);
}
