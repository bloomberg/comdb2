...
static int bdb_cursor_reposition(bdb_cursor_ifn_t *pcur_ifn, int how, int *bdberr);
static int bdb_cursor_relock_reposition(bdb_cursor_ifn_t *pcur_ifn, cursor_tran_t *curtran, int how, int *bdberr);
...
int bdb_gbl_asof_modsnap_init(bdb_state_type *bdb_state)
{
    ...
    if (rc) {
        logmsg(LOGMSG_ERROR, "%s: failed to scan logfiles. asof modsnap might not work\n", __func__);
                                 ...
                                 int how, int *retrieved, int *bdberr)
{
    ...
    *retrieved = 0;
    ...
    case DB_PREV:
        ...
        rc = bdb_cursor_relock_reposition(cur->ifn, cur->curtran, (how == DB_NEXT) ? BDB_NEXT : BDB_PREV, bdberr);
        if (rc == IX_NOTFND) {
                           ...
                           unsigned long long genid_sd, uint8_t ver_rl, int how)
{
    ...
    if (!data_sd) {
        ...
        cur->ver = ver_rl;
        /* cur->rl is callable only when the merged record came from it; the
         * temp-table paths leave it NULL and those rows read back NULL. */
        if (cur->rl)
            ...
            if (rc == IX_FND) {
                ...
                if (found_keylen < 0 || found_keylen > (int)sizeof(keybuf)) {
                    logmsg(LOGMSG_ERROR,
                           ...
                           found_keylen, sizeof(keybuf), cur->state->name, cur->type == BDBC_IX ? "ix" : "dta",
                           cur->idx, cur->genid);
                    *bdberr = BDBERR_BUG_KILLME;
                    return -1;
                }
