    ...
    private boolean reopen(boolean refresh_dbinfo_if_failed) {
        ...
        if (prefIdx != -1 && dbHostIdx != prefIdx) {
            io = new SockIO(myDbHosts.get(prefIdx),
                    pmuxrte ? overriddenPort : myDbPorts.get(prefIdx),
        ...
        if (dbHostIdx == -1 && myDbHosts.size() > 0 &&
            ...
            for (int i = 0; i != numHostsSameRoom; ++i) {
                ...
                io = new SockIO(myDbHosts.get(try_node), pmuxrte ? overriddenPort : myDbPorts.get(try_node),
                                tcpbufsz, pmuxrte ? myDbName : null,
                                soTimeout, connectTimeout);
                if (io.open()) {
                    try {
        ...
        for (; dbHostIdx < myDbHosts.size(); ++dbHostIdx) {
            ...
            io = new SockIO(myDbHosts.get(dbHostIdx), pmuxrte ? overriddenPort : myDbPorts.get(dbHostIdx),
                            tcpbufsz, pmuxrte ? myDbName : null,
                            soTimeout, connectTimeout);
            if (io.open()) {
                try {
        ...
        if (masterIndexInMyDbHosts >= 0) {
            io = new SockIO(myDbHosts.get(masterIndexInMyDbHosts),
                            pmuxrte ? overriddenPort : myDbPorts.get(masterIndexInMyDbHosts),
                            tcpbufsz, pmuxrte ? myDbName : null,
                            soTimeout, connectTimeout);
            if (io.open()) {
                try {
