...
static int writecb_int(struct event_info *e)
{
    if (evbuffer_get_length(e->flush_buf) > 0) {
        //int rc = evbuffer_write(e->flush_buf, e->fd);
        ssize_t rc = writev_evbuffer(e->flush_buf, e->fd);
    ...
    e->sent_at = time(NULL);
    size_t outstanding = evbuffer_get_length(e->flush_buf);
    if (outstanding == 0) {
...
static void writecb(int fd, short what, void *data)
{
    ...
    if (fd != e->fd) abort();
    Pthread_mutex_lock(&e->wr_lk);
    if (fd != e->fd || !e->flush_buf) abort();
    int rc = writecb_int(e);
    Pthread_mutex_unlock(&e->wr_lk);
    if (rc == 0) return;
	/* TODO: SHOULD disable or not? */
    Pthread_mutex_lock(&e->wr_lk);
    do_disable_write(e);
    Pthread_mutex_unlock(&e->wr_lk);
    reconnect(e);
