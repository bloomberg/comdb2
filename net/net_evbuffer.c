...
static void maybe_enable_accept(int fd, short what, void *data)
{
    ...
    if (total <= limit) {
        ...
        printf("%s: resuming after %lds.%ldms active:%d pending:%d evicted:%d\n", __func__, diff.tv_sec,
               diff.tv_usec / 1000, active_appsock_conns, pending_connections, evicted_appsock_conns);
    } else {
...
static void maybe_disable_accept(struct net_info *n)
{
    ...
    int limit = get_max_appsocks_limit() + 10;
    int total = active_appsock_conns + pending_connections + evicted_appsock_conns;
