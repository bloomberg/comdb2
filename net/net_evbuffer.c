...
static int portmux_server_side_validation(struct accept_info *a, char request) {
    ...
    switch(request) {
        ...
        case V_WHO:
            ...
            rc = evbuffer_reserve_space(a->buf, 2048, iov, 3);
            if (rc < 0) {
                logmsg(LOGMSG_ERROR, "%s:unable to reserve space fd:%d rc:%zd \n", __func__, a->fd, rc);
                return 1;
             }
            ...
            size = htonl(size);
            rc = evbuffer_commit_space(a->buf, iov, 3);
            if (rc < 0) {
                logmsg(LOGMSG_ERROR, "%s:unable to commit space fd:%d rc:%zd \n", __func__, a->fd, rc);
                return 1;
             }
