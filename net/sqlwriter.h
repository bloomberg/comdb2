...
typedef int (timeout_fn)(void *);
void sql_enable_timeout(struct sqlwriter *, int, timeout_fn *, void *);
...
typedef int(sql_pack_fn)(uint8_t *, void *);
int sql_write(struct sqlwriter *, int, sql_pack_fn *, void *, int);
int sql_writev(struct sqlwriter *, struct iovec *, int);

struct sqlwriter *sqlwriter_new(struct sqlclntstate *, int fd, sql_pack_fn *pack_hb, size_t hb);
...
int get_fileno(struct sqlclntstate *);
