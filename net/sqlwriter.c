...
struct sqlwriter *sqlwriter_new(struct sqlclntstate *clnt, int fd, sql_pack_fn *pack, sql_pack_fn *pack_hb, size_t sz)
{
    struct sqlwriter *writer = calloc(1, sizeof(struct sqlwriter));
    Pthread_mutex_init(&writer->wr_lock, NULL);
    writer->clnt = clnt;
    writer->pack = pack;
    writer->pack_hb = pack_hb;
    writer->hb_sz = sz;
    writer->wr_continue = 1;
    ...
    writer->heartbeat_trickle_ev = event_new(appsock_timer_base, fd, trickle_flags, sql_trickle_cb, writer);
