...
void sql_enable_trickle(struct sqlwriter *writer)
{
    event_del(writer->heartbeat_ev);
    event_add(writer->heartbeat_trickle_ev, NULL);
}

void sql_disable_trickle(struct sqlwriter *writer)
{
    event_add(writer->heartbeat_ev, &heartbeat_time);
    event_del(writer->heartbeat_trickle_ev);
}

void sql_enable_flush(struct sqlwriter *writer)
{
...
void sql_disable_flush(struct sqlwriter *writer)
{
    writer->flush = 0;
...
void sql_trickle_int(struct sqlwriter *writer, int fd)
{
...
#   if 0
    Pthread_mutex_lock(&writer->wr_lock);
    if (!writer->bad) {
        int fd = get_fileno(writer->clnt);
        ssize_t n = write(fd, 0, 0);
        if (n < 0 && errno != EAGAIN && errno != EWOULDBLOCK) {
            writer->bad = 1;
        }
    }
