...
added_to_log=$(cat qdb1-log1.log | egrep "add, <nil>, [0123456789]{1,2}$" | uniq | wc -l)

if [ $added_to_log -ne 95 ] ; then
    echo "bad queuedb side-effects, need 95 entries, got $added_to_log"
    exit 1
fi
