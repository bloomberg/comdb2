...
enum
{
    ...
    DB_INFO2_RESP_STRUCT_LEN = REQ_HDR_LEN + DB_INFO2_RESP_NO_HDR_NO_DATA_LEN
        + DBINFOPC_STRUCT_LEN 
};
BB_COMPILE_TIME_ASSERT(db_info2_resp_size,
        sizeof(struct db_info2_resp) == DB_INFO2_RESP_STRUCT_LEN);
...
enum
{
    ...
    DB_INFO2_TBL_INFO_STRUCT_LEN = DB_INFO2_TBL_INFO_NO_TBLINFO_LEN
        + TBLINFO_STRUCT_LEN
};
BB_COMPILE_TIME_ASSERT(db_info2_tbl_info_size,
        sizeof(struct db_info2_tbl_info) == DB_INFO2_TBL_INFO_STRUCT_LEN);
...
enum
{
    DB_INFO2_CLUSTER_INFO_NO_NODE_INFO_LEN = 4 + 4 + 4 + 4 + 4 + 4 + 4
        + (4 * 7),
    DB_INFO2_CLUSTER_INFO_STRUCT_LEN = DB_INFO2_CLUSTER_INFO_NO_NODE_INFO_LEN
        + NODE_INFO_LEN
};
...
BB_COMPILE_TIME_ASSERT(db_info2_stats_size, sizeof(struct db_info2_stats)
        ...
        const uint8_t *p_buf_end)
{
    ...
    p_buf = buf_put(&(p_req_hdr->ver1), sizeof(p_req_hdr->ver1), p_buf,
            p_buf_end);
    p_buf = buf_put(&(p_req_hdr->ver2), sizeof(p_req_hdr->ver2), p_buf,
            p_buf_end);
        ...
        const uint8_t *p_buf_end)
{
    ...
    p_buf = buf_put(&(p_tran_req->id), sizeof(p_tran_req->id), p_buf, 
            p_buf_end);

    return p_buf;
}

const uint8_t *tran_req_get(struct tran_req *p_tran_req,
        const uint8_t *p_buf, const uint8_t *p_buf_end)
{
    if(p_buf_end < p_buf || TRAN_REQ_LEN > (p_buf_end - p_buf))
        return NULL;

    p_buf = buf_get(&(p_tran_req->id), sizeof(p_tran_req->id), p_buf, 
            p_buf_end);
        ...
        const uint8_t *p_buf_end)
{
    ...
    p_buf = buf_put(&(p_dbinfopc->type), sizeof(p_dbinfopc->type), 
            p_buf, p_buf_end);
    p_buf = buf_put(&(p_dbinfopc->len), sizeof(p_dbinfopc->len), p_buf, 
            p_buf_end);
        ...
        const uint8_t *p_buf, const uint8_t *p_buf_end)
{
    ...
    p_buf = buf_get(&(p_dbinfopc->type), sizeof(p_dbinfopc->type), 
            p_buf, p_buf_end);
    p_buf = buf_get(&(p_dbinfopc->len), sizeof(p_dbinfopc->len), p_buf, 
            p_buf_end);
 ...
 * the end */
uint8_t *db_info2_resp_no_hdr_no_data_put(
        const struct db_info2_resp *p_db_info2_resp, uint8_t *p_buf,
        const uint8_t *p_buf_end)
{
    if(p_buf_end < p_buf
 ...
 * the end */
const uint8_t *db_info2_resp_no_hdr_no_data_get(
        struct db_info2_resp *p_db_info2_resp, const uint8_t *p_buf,
        const uint8_t *p_buf_end)
{
    if(p_buf_end < p_buf
            || DB_INFO2_RESP_NO_HDR_NO_DATA_LEN > (p_buf_end - p_buf))
...
const uint8_t *db_info2_tbl_info_no_tblinfo_get(
        struct db_info2_tbl_info *p_db_info2_tbl_info, const uint8_t *p_buf,
        const uint8_t *p_buf_end)
{
    if(p_buf_end < p_buf
            || DB_INFO2_TBL_INFO_NO_TBLINFO_LEN > (p_buf_end - p_buf))
...
const uint8_t *keyinfo_get(struct keyinfo *p_keyinfo,
        const uint8_t *p_buf, const uint8_t *p_buf_end)
{
    if(p_buf_end < p_buf || KEYINFO_LEN > (p_buf_end - p_buf))
        return NULL;
...
uint8_t *db_info2_cluster_info_no_node_info_put(
        const struct db_info2_cluster_info *p_db_info2_cluster_info,
        uint8_t *p_buf, const uint8_t *p_buf_end)
{
    if(p_buf_end < p_buf
    ...
    p_buf = buf_put(&(p_db_info2_cluster_info->syncmode),
            sizeof(p_db_info2_cluster_info->syncmode), p_buf, p_buf_end);
    p_buf = buf_put(&(p_db_info2_cluster_info->formkey_enabled),
            sizeof(p_db_info2_cluster_info->formkey_enabled), p_buf, p_buf_end);
    ...
    p_buf = buf_put(&(p_db_info2_cluster_info->nsiblings),
            sizeof(p_db_info2_cluster_info->nsiblings), p_buf, p_buf_end);
    p_buf = buf_put(&(p_db_info2_cluster_info->incoherent),
...
const uint8_t *db_info2_cluster_info_no_node_info_get(
        struct db_info2_cluster_info *p_db_info2_cluster_info,
        const uint8_t *p_buf, const uint8_t *p_buf_end)
{
    if(p_buf_end < p_buf
            || DB_INFO2_CLUSTER_INFO_NO_NODE_INFO_LEN > (p_buf_end - p_buf))
    ...
    p_buf = buf_get(&(p_db_info2_cluster_info->master),
            sizeof(p_db_info2_cluster_info->master), p_buf, p_buf_end);
    p_buf = buf_get(&(p_db_info2_cluster_info->syncmode),
            sizeof(p_db_info2_cluster_info->syncmode), p_buf, p_buf_end);
    ...
    p_buf = buf_get(&(p_db_info2_cluster_info->port),
            sizeof(p_db_info2_cluster_info->port), p_buf, p_buf_end);
    p_buf = buf_get(&(p_db_info2_cluster_info->nsiblings),
            sizeof(p_db_info2_cluster_info->nsiblings), p_buf, p_buf_end);
    p_buf = buf_get(&(p_db_info2_cluster_info->incoherent),
            sizeof(p_db_info2_cluster_info->incoherent), p_buf, p_buf_end);
...
/* struct tm is 44 bytes on linux & 36 bytes everywhere else */
#ifndef _LINUX_SOURCE
BB_COMPILE_TIME_ASSERT(client_datetime_size,
        sizeof(struct client_datetime) == CLIENT_DATETIME_LEN);
#endif
...
uint8_t *client_extended_datetime_put(const client_datetime_t *p_client_datetime,
        uint8_t *p_buf, const uint8_t *p_buf_end)
{
    if(p_buf_end < p_buf || CLIENT_DATETIME_LEN > (p_buf_end - p_buf))
        return NULL;

    p_buf = tm_extended_put(&(p_client_datetime->tm), p_buf, p_buf_end);
    p_buf = buf_put(&(p_client_datetime->msec), sizeof(p_client_datetime->msec),
            p_buf, p_buf_end);
    p_buf = buf_no_net_put(&(p_client_datetime->tzname), 
            sizeof(p_client_datetime->tzname), p_buf, p_buf_end);
...
uint8_t *client_intv_ym_put(const client_intv_ym_t *p_client_intv_ym, 
        uint8_t *p_buf, const uint8_t *p_buf_end)
{
...
uint8_t *client_intv_ds_put(const client_intv_ds_t *p_client_intv_ds, 
        uint8_t *p_buf, const uint8_t *p_buf_end)
{
...
const uint8_t *client_intv_ds_get(client_intv_ds_t *p_client_intv_ds, const 
        uint8_t *p_buf, const uint8_t *p_buf_end)
{
    if(p_buf_end < p_buf || CLIENT_INTV_DS_LEN > (p_buf_end - p_buf))
        return NULL;
...
const uint8_t *client_intv_ym_get(client_intv_ym_t *p_client_intv_ym, const uint8_t
        *p_buf, const uint8_t *p_buf_end)
{
    if(p_buf_end < p_buf || CLIENT_INTV_YM_LEN > (p_buf_end - p_buf))
        return NULL;
