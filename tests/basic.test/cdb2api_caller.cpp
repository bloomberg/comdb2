...
int main(int argc, char *argv[])
{
    ...
    for (int j = 2000; j < 3000; j++) {
        ...
        unsigned char palltypes_blob[17] = "1234567890123456";

        if(cdb2_bind_param(db, "palltypes_short", CDB2_INTEGER, &i, sizeof(i)) )
        ...
        if(cdb2_bind_param(db, "palltypes_blob", CDB2_BLOB, palltypes_blob, sizeof(palltypes_blob)-1))
            fprintf(stderr, "Error binding palltypes_blob.\n");
        types.push_back(CDB2_INTEGER);

        ss << "insert into t1(alltypes_short, alltypes_byte, alltypes_blob) values (@palltypes_short, @palltypes_byte, @palltypes_blob)" ;
