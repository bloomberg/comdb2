...
for testcase in $files ; do
    ...
    #check if the dbs are still up
    cdb2sql -s --cdb2cfg ${a_cdb2config} $a_dbname default 'select 1'
    if (( "$?" != "0" )) ; then
        echo "Failure, db $a_dbname down testcase $testcase"
        exit 1
    fi
    cdb2sql -s --cdb2cfg ${a_remcdb2config} $a_remdbname default 'select 1'
    if (( "$?" != "0" )) ; then
        echo "Failure, remdb $a_remdbname down testcase $testcase"
        exit 1
    fi
