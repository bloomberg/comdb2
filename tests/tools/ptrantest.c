...
static int set_update_value(long long val, FILE *f)
{
    ...
    if(verbose) {
        fprintf(f,"Thread %" PRIxPTR " updating current seed to %lld\n", (intptr_t)pthread_self(), val);
    }
    ...
    if(val == updated_value) {
        if(verbose) {
            fprintf(stderr,"Error!  Thread %" PRIxPTR " succeeded in updating to current value (%lld)?\n",
                    (intptr_t)pthread_self(), val);
        }
        rc=1;
    }
...
static void *updater(void *arg)
{
    ...
    if(0 != rc) {
        fprintf(stderr,"cdb2_open failed for thread %" PRIxPTR "\n", (intptr_t)pthread_self());
        exit_pthread=1;
        pthread_exit((void *)1);
        exit(1);
    }
    ...
    while(iterations > 0) {
        ...
        if(CDB2_OK != rc) {
            fprintf(stderr,"Thread %" PRIxPTR " error finding record rc=%d %s\n",
        ...
        while(CDB2_OK == rc) {
            val = *(long long*) cdb2_column_value(sqlh, 0);
            fprintf(stderr,"Thread %" PRIxPTR " found another record with seed=%lld\n",
                    (intptr_t)pthread_self(), val);
            rc = cdb2_next_record( sqlh );
        }
        ...
        if(rc) {
            fprintf(stderr,"Thread %" PRIxPTR " error on update rc=%d %s\n",(intptr_t)pthread_self(),
                    rc, cdb2_errstr( sqlh ));
        ...
        rc = cdb2_next_record( sqlh );
        if(rc) {
            fprintf(stderr,"Thread %" PRIxPTR "error on next-rec, rc=%d %s\n",
                    (intptr_t)pthread_self(), rc, cdb2_errstr( sqlh ));
            exit_pthread=1;
            pthread_exit((void *)1);
        }
        ...
        if(nupd < 0 || nupd > 1) {
            fprintf(stderr,"Thread %" PRIxPTR " updated %d records??\n", (intptr_t)pthread_self(),
                    nupd);
            exit_pthread=1;
            pthread_exit((void *)1);
        }
        ...
        if(rc) {
            fprintf(stderr,"Thread %" PRIxPTR " updated to %lld but the last updated value was already %lld???\n",
...
int main(int argc,char *argv[])
{
    ...
    for(i = 0 ; i < num_threads ; i++) {
        ...
        if(ret) {
            fprintf(stderr,"ERROR FROM THREAD #%" PRIxPTR "\n", (intptr_t)tids[i]);
        }
        joinrc += ret;
    }
