...
int create_consistent_hash(int numcopies, int numshards, hash_func func) {
    ch = ch_hash_create(numcopies, func);
    if (!ch) {
    ...
    for(int i=1; i<=numshards; i++) {
        ...
        if (ch_hash_add_node(ch, (uint8_t *)shard, sizeof(shard))) {
...
void usage(const char *p, const char *err) {
    fprintf(stderr, "%s\n", err);
    fprintf(stderr, "Usage %s --numshards NUMSHARDS --numcopies NUMCOPIES --numrecords NUMRECORDS\n", p);
    exit(1);
}
...
int main(int argc, char *argv[]) {
    ...
    int numshards = 0, numcopies = 0, numrecords = 0;
    static struct option long_options[] =
    {
        {"numshards", required_argument, NULL, 's'},
        {"numcopies", required_argument, NULL, 'c'},
        {"numrecords", required_argument, NULL, 'r'},
        {NULL, 0, NULL, 0}
    };
    int c;
    int index;
    while ((c = getopt_long(argc, argv, "s:c:r:?", long_options, &index)) != -1) {
        //printf("c '%c' %d index %d optarg '%s'\n", c, c, index, optarg);
        switch(c) {
            case 's': numshards = atoi(optarg); break;
            case 'c': numcopies = atoi(optarg); break;
    ...
    printf("numshards: %d, numcopies: %d, numrecords: %d\n", numshards, numcopies, numrecords);

    sqlite3 *db;
    char *err_msg = NULL;
    int rc = 0;
    if (create_consistent_hash(numcopies, numshards, ch_hash_sha)) {
