...
static int readers_done; /* readers -> body: the fixed iteration budget is spent */

/* Readers run this many 16-scan iterations per phase, rather than "as many as
   fit in a fixed window".  The window and one iteration were the same order of
   magnitude, so whether a last iteration started was decided by a few percent
   of scan speed -- and that doubled both rd_scans and phase_ms, making arms
   incomparable.  A fixed budget makes the reader work identical in every arm. */
static int reader_iterations = 2;
...
/* Read-only mode (-r): run the readers with no writer at all, to price the
   waiter checking and release machinery when there is nothing to contend with.
   Only the time-bounded phase runs; see runit(). */
static int readonly_mode;
...
struct lockstats {
    ...
    long long repl_wait_us, repl_waits;
    ...
    long long probe_us, probes, probes_timed;
};
...
static void metrics_report(const char *phase, long long phase_ms, const struct lockstats *lk)
{
    ...
    /* Appended only when the counters were actually sampled (-l), so a run
       ...
       like a measurement. */
    char lkbuf[1024]; /* 20 fields at their int64 widest come to ~700 */
    lkbuf[0] = 0;
    if (lk)
        snprintf(lkbuf, sizeof(lkbuf),
                 " lk_rd_wait_us=%lld lk_rd_waits=%lld lk_wr_wait_us=%lld lk_wr_waits=%lld"
                 " lk_repl_wait_us=%lld lk_repl_waits=%lld lk_other_wait_us=%lld lk_other_waits=%lld"
                 ...
                 " lk_probe_us=%lld lk_probes=%lld lk_probes_timed=%lld lk_probe_est_us=%lld",
                 lk->rd_wait_us, lk->rd_waits, lk->wr_wait_us, lk->wr_waits,
                 lk->repl_wait_us, lk->repl_waits, lk->other_wait_us, lk->other_waits,
                 ...
                 lk->probe_us, lk->probes, lk->probes_timed,
                 /* only one probe in N is timed, so scale the measured time by
...
static void lockstats_add_host(struct lockstats *acc, const char *host)
{
    static const struct {
        ...
        {"lockwait_repl_time", offsetof(struct lockstats, repl_wait_us)},
        {"lockwait_repl_count", offsetof(struct lockstats, repl_waits)},
        ...
        {"lockprobe_time", offsetof(struct lockstats, probe_us)},
        {"lockprobe_count", offsetof(struct lockstats, probes)},
        {"lockprobe_sampled", offsetof(struct lockstats, probes_timed)},
    };
...
static void *readers(void *data)
{
    ...
    gettimeofday(&start, NULL);
    int count = 0;
    /* A fixed budget: every arm reads exactly this much, no exceptions.  Not
       even insert cuts it short -- insert is not a tabled phase, so the extra
       time it costs there is pure overhead, symmetric across arms, and never
       shows up in a number anyone compares. */
    while (count < reader_iterations) {
        ++count;
        struct timeval start, finish, elapsed;
        gettimeofday(&start, NULL);
...
static void update(void)
{
    ...
    do {
        ...
        /* Deliberately NOT "until the readers are done": coupling the writer to
           the readers makes the two drive each other.  The feature slows
           readers, which would extend the writer, which adds contention, which
           slows readers further -- a feedback loop that turned a 5.5 minute arm
           into 15+.  A fixed window keeps the write load identical in every arm,
...
/* Readers only.  Holds the phase open for the same 60s window update() uses so
   the reader numbers stay comparable, but issues no writes -- with no writer
   there is no contention, so whatever the release machinery costs here is what
   it costs when it has nothing to do. */
static void readonly_body(void)
{
    Printf("%s started\n", __func__);
    struct timeval start, finish, elapsed;
    gettimeofday(&start, NULL);
...
static int runit(void)
{
    ...
    /* -r keeps only the time-bounded phase, with the writer removed.  The
       work-bounded delete phase has nothing to measure without writes. */
    if (readonly_mode) {
        run_phase("update", readonly_body, 1);
...
int main(int argc, char **argv)
{
    ...
    /* usage: reco-ddlk-sql [-p] [-l] [-r] [-i iters] dbname [tier] */
    int npos = 0;
    for (int i = 1; i < argc; ++i) {
        if (strcmp(argv[i], "-p") == 0) {
            ...
            reader_iterations = atoi(argv[++i]);
            if (reader_iterations < 1) reader_iterations = 1;
        } else if (npos == 0) {
            dbname = argv[i];
            npos = 1;
        } else if (npos == 1) {
    ...
    if (!dbname) {
        fprintf(stderr, "usage: %s [-p] [-l] [-r] [-i iters] dbname [tier]\n", argv[0]);
        return 1;
    }
