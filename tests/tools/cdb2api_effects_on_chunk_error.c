...
int value_comparer(cdb2_hndl_tp *hndl, int divider, int divider2, int divider3, int *print_error) {
...
int test1(char *dbname) {
    ...
    if (rc)
        goto err;
    rc = value_comparer(hndl, 96, -1, -1, &print_error);
    if (rc)
        goto err;
...
int test2(char *dbname, int continue_on_verify_error) {
    ...
    if (rc)
        goto err;
    if (continue_on_verify_error) // the last chunk also gets rolled back
        rc = value_comparer(hndl, 16, 32, 96, &print_error);
    else
        rc = value_comparer(hndl, 16, -1, -1, &print_error);
    if (rc)
        goto err;
