...
void likely(int rc, int expected)
{
    if (rc != expected) fail();
}
    ...
    while (1) {
        ...
        case CDB2_OK:
            ...
            case CDB2_CSTRING:
                ...
                break;
            default: fail(hndl, "cdb2_run_statement");
            }
            break;
...
int main(int argc, char **argv)
{
    ...
    likely(cdb2_push_context(hndl, "1"), 0);
    exec(hndl, "SELECT 1");

    /* Fill the stack buffer. */
    for (i = 2; i <= 10; i++) {
        sprintf(buf, "%d", i);
        likely(cdb2_push_context(hndl, buf), 0);
    }
    exec(hndl, "SELECT 1");

    /* Check stack buffer overflow. */
    likely(cdb2_push_context(hndl, "MUST NOT BE LOGGED!"), 1);
    exec(hndl, "SELECT 1");

    /* Remove the message at the top. */
    likely(cdb2_pop_context(hndl), 0);
    exec(hndl, "SELECT 1");

    /* Clear all context messages. */
    likely(cdb2_clear_contexts(hndl), 0);

    /* Check stack buffer underflow. */
    likely(cdb2_push_context(hndl, "1"), 0);
    likely(cdb2_pop_context(hndl), 0);
    likely(cdb2_pop_context(hndl), 1);
    exec(hndl, "SELECT 1");
