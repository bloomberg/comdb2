...
function process_node() {
    ...
    $CDB2SQL_EXE --tabs ${CDB2_OPTIONS} $dbnm --host $node "exec procedure sys.cmd.analyze('t1')"  &> t04_01.req.res.0
    ...
    $CDB2SQL_EXE ${CDB2_OPTIONS} $dbnm --host $node  "exec procedure sys.cmd.send('analyze thresh 1000')" &> t04_01.req.res.1

    #set headroom to current disk headroom
    headroom=$((100-currused+1))
    $CDB2SQL_EXE ${CDB2_OPTIONS} $dbnm --host $node "exec procedure sys.cmd.send('analyze headroom $headroom')" &> t04_01.req.res.2

    #run analyze
    $CDB2SQL_EXE --tabs ${CDB2_OPTIONS} $dbnm --host $node "exec procedure sys.cmd.analyze('t1')"  &> t04_01.req.res.3
    ...
    $CDB2SQL_EXE ${CDB2_OPTIONS} $dbnm --host $node "exec procedure sys.cmd.send('analyze headroom 6')" &> t04_01.req.res.4

    #run analyze
    $CDB2SQL_EXE --tabs ${CDB2_OPTIONS} $dbnm --host $node "exec procedure sys.cmd.analyze('t1')"  &> t04_01.req.res.5
    ...
    $CDB2SQL_EXE ${CDB2_OPTIONS} $dbnm --host $node "exec procedure sys.cmd.send('analyze thresh 1000000')" &> t04_01.req.res.6
}
