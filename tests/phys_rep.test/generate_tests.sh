...
printf "create procedure foo version '2' { local function main(x) db:emit(x) db:emit(x) end }" > 4-2-sp-add.src.sql
...
let j=j+1
printf "alter table t2 options { $(cat alltypes_full.csc2) } \$\$\n" > 5-5-alter-opts.src.sql
...
printf "drop table if exists t1\ndrop table if exists t2" > 6-1-drop.src.sql
...
printf 'select * from t2' | tee 5-1-create-alltypes.query.sql 5-2-alter.query.sql \
    5-3-alter-opts.query.sql 5-4-alter-opts.query.sql 5-5-alter-opts.query.sql

printf "select * from comdb2_tables" > 6-1-drop.query.sql

# TEST 4
printf 'select * from comdb2_procedures' | tee 4-1-sp-create.query.sql 4-2-sp-version.query.sql 4-2-sp-add.query.sql
