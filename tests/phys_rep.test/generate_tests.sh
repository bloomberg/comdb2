...
printf "alter table t2 options rec none, blobfield none, ipu off, isc off, odh off, rebuild { $(cat alltypes_full.csc2) }" > 5-3-alter-opts.src.sql
let j=$NRECS+1
...
let j=j+1
printf "alter table t2 options rec none, blobfield none, ipu off, isc off, rebuild { $(cat alltypes_full.csc2) }" > 5-4-alter-opts.src.sql
...
printf 'select * from t2' | tee 5-1-create-alltypes.query.sql 5-2-alter.query.sql \
    5-3-alter-opts-query.sql 5-4-alter-opts.query.sql
...
printf "exec procedure foo('hello!')" > 4-3-sp-version.src.sql
