...
{
    int off = snprintf(out, outlen, "%s:origin:%s:pid:%d", clnt->argv0 ? clnt->argv0 : "?",
                       clnt->origin ? clnt->origin : "?", clnt->conninfo.pid);
...
int access_control_check_write(struct ireq *iq, tran_type *trans, int *bdberr)
{
    ...
    if (gbl_uses_externalauth && iq->usedb && externalComdb2AuthenticateUserWrite) {
...
int access_control_check_read(struct ireq *iq, tran_type *trans, int *bdberr)
{
    ...
    if (gbl_uses_externalauth && iq->usedb && externalComdb2AuthenticateUserRead) {
