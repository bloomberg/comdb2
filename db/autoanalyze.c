...
int load_auto_analyze_counters(void)
{
    ...
    for (int i = 0; i < thedb->num_dbs; i++) {
        ...
        tbl->aa_lastepoch = 0;
        if (save_freq > 0) {
            get_saved_counter_epoch(tbl->tablename_ip, &tbl->aa_saved_counter,
                                    &tbl->aa_lastepoch);
...
static long long get_num_rows_from_stat1(struct dbtable *tbldb)
{
    ...
    rc =
        stat1_ondisk_record(&iq, tbldb->tablename_ip, ix_txt, NULL, (void **)&rec);
    if (rc != 0) {
        logmsg(LOGMSG_ERROR,
        ...
        if (!auto_analyze_running && !gbl_schema_change_in_progress &&
            ...
            if (bdb_attr_get(thedb->bdb_attr, BDB_ATTR_AA_REQUEST_MODE)) {
                ctrace("AUTOANALYZE: Requesting analyze be run for Table %s, "
                       "counters (%d, %d); last "
                       "run time %s\n",
                       tbl->tablename_ip, tbl->aa_saved_counter, delta,
                       ctime_r(&tbl->aa_lastepoch, my_buf));
