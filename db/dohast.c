...
int gbl_dohast_disable_joins = 1;
                              ...
                              struct params_info **pParamsOut, int is_union)
{
    ...
    if (gbl_dohast_disable_joins && p->pSrc->nSrc > 1)
...
static dohsql_node_t *gen_select(Vdbe *v, Select *p)
{
    ...
    /* no with or subqueries (joins conditional) */
    if (not_recognized || p->pSrc->nSrc == 0 /*with*/ ||
        (gbl_dohast_disable_joins && p->pSrc->nSrc > 1) /*joins*/ ||
