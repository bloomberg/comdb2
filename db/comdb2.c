...
int gbl_principal_authz_cache_enabled = 0;
...
int gbl_tolerate_missing_keys_on_delete;
...
static int init(int argc, char **argv)
{
     ...
     * line. This flows on to gbl_dbname, dbenv->envname and all the
     * lrl/directory paths derived from dbname below. In import mode dbname is a
     * read-only literal (and already lower case), so skip it. */
    if (!gbl_import_mode) {
