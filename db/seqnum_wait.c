...
int add_to_seqnum_wait_queue(bdb_state_type *bdb_state, seqnum_type *seqnum, struct ireq *iq)
{
    ...
    swait->target_host = iq->sorese->target_host;
    swait->rqid = iq->sorese->rqid;
    comdb2uuidcpy(swait->uuid, iq->sorese->uuid);
    if ((swait->have_snap = (iq->sorese->snap_info != NULL)) != 0)
...
{
    /* The bdb write lock is wanted -- a role change (up/downgrade) or an env
     * reopen/recovery is stopping the world, so we can no longer prove this
     * commit is durable here.  Abandon the wait, exactly as the inline path's
     * lock-desired early exits do (bdb/rep.c), and mirror their commit
     * accounting so commits in flight aren't lost from the counters. */
    ATOMIC_ADD64(gbl_distributed_commit_count, 1);
    ATOMIC_ADD64(gbl_not_durable_commit_count, 1);
    item->outrc = item->durable_lsns ? ERR_NOT_DURABLE : -1;
...
static void process_work_item(struct seqnum_wait *item)
{
    ...
    case SEQNUM_WAIT_INIT:
        /* Normally 0: trans_commit_int() refuses to enqueue when any of these
         * is set.  Re-read anyway -- they are runtime tunables and can come on
         * between the enqueue and here, and then the durability accounting
    ...
    done_wait:
        ...
        /* toblock does this for the inline path */
    ...
    case SEQNUM_WAIT_COMMIT:
        if (bdb_attr_get(bdb_state->attr, BDB_ATTR_COHERENCY_LEASE)) {
             ...
             * Re-checked on every entry -- an ack for an unrelated commit pulls
             * us back in through the LSN list, which does not honour next_ts. */
    ...
    commit: {
        ...
        /* if the transaction has an error, report it, otherwise report the commit's durability status.
         * unless the commit is not durable, in which case we overwrite the error code since we want
         * to report that to the replicant so it retries. */
        ...
        osql_comm_signal_sqlthr_rc(item->target_host, item->rqid, item->uuid, item->nops, &item->errstat,
                                   item->have_snap ? &item->snap : NULL, sorese_rc);
...
/* Process every item that is ready, freeing the finished ones.  'ts_order'
 * picks the list to walk: by deadline if we woke on a timeout, by LSN if we
 * woke on an ack.  Both are sorted, so the walk stops at the first item that
 * is not ready yet. */
...
/* The seqnum-wait thread: drain whatever is ready, then sleep until the next
 * ack lands or the earliest deadline expires, whichever comes first.  Which of
 * the two woke us decides which list drain() walks next time around. */
static void *queue_processor(void *arg)
{
    ...
    while (1) {
        ...
        /* Park on new_lsns_cond, guarded by the lock that owns new_lsns, so an
         * incoming ack's bump+broadcast cannot land between the check and the
         * sleep.  A timed wait still bounds us by the item's deadline. */
        Pthread_mutex_lock(&max_lsn_so_far_lk);
        ...
        wait_rc = pthread_cond_timedwait(&new_lsns_cond, &max_lsn_so_far_lk, &waittime);
        local_new_lsns = new_lsns;
        Pthread_mutex_unlock(&max_lsn_so_far_lk);
    }
    return NULL;
