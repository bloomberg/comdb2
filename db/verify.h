...
typedef enum { VERIFY_DEFAULT, VERIFY_PARALLEL, VERIFY_DATA, VERIFY_INDICES, VERIFY_BLOBS} verify_mode_t;
...
int verify_table(const char *table, SBUF2 *sb, int progress_report_seconds,
             int attempt_fix, 
             int (*lua_callback)(void *, const char *), void *lua_params, verify_mode_t mode);
