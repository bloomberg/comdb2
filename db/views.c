...
/**
 * Common checks for a retention change; "view" is the live partition (NULL if
 * it is gone) and the caller must hold views_lk.
 */
static int _validate_retention_change(timepart_view_t *view, const char *name, int new_retention, int is_manual,
                                      struct errstat *err)
{
    if (!view) {
        errstat_set_rcstrf(err, VIEW_ERR_EXIST, "Time partition %s was removed", name);
        return VIEW_ERR_EXIST;
    }

    if (view->rolltype != TIMEPART_ROLLOUT_TRUNCATE) {
        errstat_set_rcstrf(err, VIEW_ERR_PARAM, "Partition %s is a legacy partition, use PUT instead", name);
        return VIEW_ERR_PARAM;
    }

    if ((view->period == VIEW_PARTITION_MANUAL) != (is_manual != 0)) {
        errstat_set_rcstrf(err, VIEW_ERR_PARAM, "Partition %s is %s, use PARTITIONED BY %s", name,
                           view->period == VIEW_PARTITION_MANUAL ? "manual" : "time",
                           view->period == VIEW_PARTITION_MANUAL ? "MANUAL" : "TIME");
        return VIEW_ERR_PARAM;
    }

    if (new_retention < 2 || new_retention >= VIEWS_MAX_RETENTION) {
        errstat_set_rcstrf(err, VIEW_ERR_PARAM, "Invalid retention %d for partition %s", new_retention, name);
        return VIEW_ERR_PARAM;
    }

    if (new_retention == view->retention) {
        errstat_set_rcstrf(err, VIEW_ERR_PARAM, "Partition %s retention is already %d", name, new_retention);
        return VIEW_ERR_PARAM;
    }
 ...
 * retention must actually change.
 * Used both when the alter is processed and again during finalize (under the
 * schema lock) to catch a partition removed in between.
 */
int timepart_validate_retention_change(const char *name, int new_retention, int is_manual, struct errstat *err)
{
    ...
    Pthread_rwlock_rdlock(&views_lk);
    rc = _validate_retention_change(_get_view(thedb->timepart_views, name), name, new_retention, is_manual, err);
    Pthread_rwlock_unlock(&views_lk);
                                   ...
                                   int *is_increase_out, struct errstat *err)
{
    ...
    *is_increase_out = 0;
    ...
    view = _get_view(thedb->timepart_views, name);
    rc = _validate_retention_change(view, name, new_retention, is_manual, err);
    if (rc != VIEW_NOERR)
        goto done_locked;
    ...
    /* NOTE: current_shard is not persisted, it is recomputed on reload by
     * _view_find_current_shard(); that lookup ignores never-used (INT_MAX)
     * shards, so the array order below does not affect the ring position. */
    ...
    if (is_increase) {
        ...
        /* old shards, in rollout order, follow the new empties; the current
         * shard (old_order[old_retention-1]) lands last, so the new empties
         * are the next ones consumed */
        ...
        /* keep the surviving shards in rollout order (oldest kept first, the
         * current shard last); the dropped shards' data is discarded and the
         * survivors keep their existing [low,high) coverage unchanged */
...
static void _view_find_current_shard(timepart_view_t *view)
{
     ...
     * NOTE: a shard that was never rolled into has low == INT_MAX and can
     * never be the current one; skipping those keeps the result independent
     * of the position such shards occupy in the array.
     */
    for (i = 0; i < view->nshards; i++) {
        ...
        if (view->shards[i].low == INT_MAX)
            continue;
        if (newest_earlier_shard < 0 || view->shards[newest_earlier_shard].low < view->shards[i].low)
            newest_earlier_shard = i;
    }
    if (newest_earlier_shard < 0) {
        /* no shard was ever used; the first one is next in line */
        newest_earlier_shard = 0;
    }
