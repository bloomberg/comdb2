 ...
 *
 */
static int _table_exists(fdb_t *fdb, const char *table_name, int *version)
{
    fdb_tbl_t *table;
    int exists = 0;
...
static int __check_sqlite_stat(sqlite3 *db, fdb_tbl_ent_t *ent, Table *tab)
{
    ...
    if (unlikely(ent && tab && (tab->version != ent->tbl->version))) {
        logmsg(LOGMSG_ERROR, "Stale cache for \"%s.%s\", sql version=%u != shared version=%llu\n",
                   ...
                   fdb_tbl_ent_t **p_ent)
{
    ...
    *p_ent = NULL;
    ...
    if (!ent || ent->tbl->version != tab->version) {
        ...
        if (gbl_fdb_track) {
            if (ent) {
                logmsg(LOGMSG_USER, "Stale cache for \"%s.%s\", sql version=%d != "
                                "shared version=%llu\n",
