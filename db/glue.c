                                 ...
                                 uint8_t _)
{
    if (!valid_trigger_reg_payload(dtap, dtalen)) {
        logmsg(LOGMSG_ERROR, "%s: invalid trigger registration payload length %d\n", __func__, dtalen);
        if (hndl)
            net_ack_message(hndl, CDB2_TRIG_ASSIGNED_OTHER);
        return;
    }
...
{
    if (!valid_trigger_reg_payload(dtap, dtalen)) {
        logmsg(LOGMSG_ERROR, "%s: invalid trigger unregister payload length %d\n", __func__, dtalen);
        if (hndl)
            net_ack_message(hndl, CDB2_TRIG_ASSIGNED_OTHER);
        return;
    }
