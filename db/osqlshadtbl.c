                                             ...
                                             int osql_nettype)
{
    ...
    rc = osql_send_fingerprint(osql->target_host, osql->rqid, osql->uuid, fingerprint, osql_nettype);
    if (rc)
        return rc;
    ...
    while (rc == 0) {
        ...
        if (datalen == 0) { /* this a delete, not an update */
            ...
            if (osql->is_reorder_on) {
                rc = osql_send_delrec(osql->target_host, osql->rqid, osql->uuid,
                                      genid,
                                         ...
                                         int *bdberr, unsigned long long seq)
{
    ...
    rc = osql_send_updcols(osql->target_host, osql->rqid, osql->uuid, savkey,
                                       ...
                                       char *record)
{
    ...
    for (i = 0; i < tbl->nblobs; i++) {
        ...
        rc = osql_send_qblob(osql->target_host, osql->rqid, osql->uuid, idx, seq,
                                       ...
                                       unsigned long long seq, int is_delete)
{
    ...
    for (i = 0; i < tbl->nix; i++) {
        ...
        rc = osql_send_index(osql->target_host, osql->rqid, osql->uuid, seq,
                                     ...
                                     int *bdberr, int crt_nops)
{
    ...
    while (rc == 0) {
        ...
        if (osql->is_reorder_on) {
            rc = osql_send_insrec(osql->target_host, osql->rqid, osql->uuid, key,
                                  (gbl_partial_indexes && tbl->ix_partial)
                                     ...
                                     int *bdberr, int crt_nops)
{
    ...
    while (rc == 0) {
        ...
        if (osql->is_reorder_on) {
            rc = osql_send_updrec(osql->target_host, osql->rqid, osql->uuid, genid,
                                  (gbl_partial_indexes && tbl->ix_partial)
                                 ...
                                 int64_t nKey, int *bdberr)
{
    ...
    for (ix = 0; ix < pCur->db->nix; ix++) {
        ...
        tmpcur =
            bdb_cursor_open(pCur->db->handle, thd->clnt->dbtran.cursor_tran, thd->clnt->dbtran.shadow_tran, ix,
                            BDB_OPEN_SHAD, osql_get_shadtbl_addtbl_newcursor(pCur), 0, 0, NULL, NULL, NULL, NULL, NULL,
                            thd->clnt->bdb_osql_trak, bdberr, (thd->clnt->dbtran.mode == TRANLEVEL_SNAPISOL));
        if (tmpcur == NULL) {
                                 ...
                                 struct sql_thread *thd, int *bdberr)
{
    ...
    for (ix = 0; ix < db->nix; ix++) {
        ...
        assert(thd->clnt->dbtran.mode != TRANLEVEL_SNAPISOL || thd->clnt->modsnap_in_progress);
        tmpcur =
            bdb_cursor_open(db->handle, thd->clnt->dbtran.cursor_tran, thd->clnt->dbtran.shadow_tran, ix, BDB_OPEN_SHAD,
                            osql_get_shadtbl_addtbl_newcursor(pCur), 0, 0, NULL, NULL, NULL, NULL, NULL,
                            thd->clnt->bdb_osql_trak, bdberr, (thd->clnt->dbtran.mode == TRANLEVEL_SNAPISOL));
        if (tmpcur == NULL) {
            logmsg(LOGMSG_ERROR, "%s:bdb_cursor_open ix %d rc %d\n", __func__, ix, *bdberr);
            return -1;
...
static int process_local_shadtbl_sc(struct sqlclntstate *clnt, int *bdberr)
{
    ...
    while (rc == 0) {
        ...
        rc = osql_send_schemachange(osql->target_host, osql->rqid, osql->uuid, sc,
                                    NET_OSQL_SOCK_RPL);
        if (rc) {
            logmsg(LOGMSG_ERROR,
...
static int process_local_shadtbl_bpfunc(struct sqlclntstate *clnt, int *bdberr)
{
    ...
    while (rc == 0) {
        ...
        rc = osql_send_bpfunc(osql->target_host, osql->rqid, osql->uuid, func->arg,
                              NET_OSQL_SOCK_RPL);
        free_bpfunc(func);
