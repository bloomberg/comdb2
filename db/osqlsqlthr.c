                               ...
                               int start_flags)
{
    ...
    if (!keep_rqid) {
        ...
        if (gbl_debug_disttxn_trace) {
            uuidstr_t us;
            logmsg(LOGMSG_USER, "DISTTXN REPL %s %s starting uuid %s\n", __func__, clnt->dist_txnid ? clnt->dist_txnid : "(nodisttxn)", comdb2uuidstr(osql->uuid, us));
        }
    } else {
        if (gbl_debug_disttxn_trace) {
            uuidstr_t us;
...
static int osql_wait(struct sqlclntstate *clnt)
{
    ...
    if (gbl_debug_disttxn_trace) {
        uuidstr_t us;
        logmsg(LOGMSG_USER, "DISTTXN REPL %s %s took %d ms to commit rqid=%llu uuid=%s\n", __func__, clnt->dist_txnid? clnt->dist_txnid : "(nodisttxn)", (endms - startms), osql->rqid,
               comdb2uuidstr(osql->uuid, us));
    }
    return rc;
}
...
int osql_sock_commit(struct sqlclntstate *clnt, int type, enum trans_clntcomm sideeffects)
{
    ...
    if (gbl_debug_disttxn_trace) {
        uuidstr_t us;
        logmsg(LOGMSG_USER, "DISTTXN REPL %s %s use_2pc %d start%s uuid=%s\n", __func__, clnt->dist_txnid ? clnt->dist_txnid : "(nodisttxn)",
               clnt->use_2pc, clnt->dbtran.dtran ? " has remote writes" : " no remote writes", comdb2uuidstr(clnt->osql.uuid, us));
    }
...
err:
    ...
    if (gbl_debug_disttxn_trace) {
        uuidstr_t us;
        logmsg(LOGMSG_USER, "DISTTXN REPL %s %s rc %d uuid=%s\n", __func__, clnt->dist_txnid ? clnt->dist_txnid : "(nodisttxn)", rc,
               comdb2uuidstr(clnt->osql.uuid, us));
    }
...
int osql_sock_abort(struct sqlclntstate *clnt, int type)
{
    ...
    if (gbl_debug_disttxn_trace) {
        uuidstr_t us;
        logmsg(LOGMSG_USER, "DISTTXN REPL %s %s rc %d uuid=%s\n", __func__, clnt->dist_txnid ? clnt->dist_txnid : "(nodisttxn)", rc, comdb2uuidstr(clnt->osql.uuid, us));
    }
