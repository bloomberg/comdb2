...
retry:
    ...
    rc = osql_comm_send_socksqlreq(osql->target_host, clnt->sql,
...
static int osql_send_del_logic(struct BtCursor *pCur, struct sql_thread *thd)
{
    ...
    if (osql->is_reorder_on) {
        rc = osql_send_delrec(
            osql->target_host, osql->rqid, osql->uuid, pCur->genid,
                               ...
                               blob_buffer_t *blobs, int maxblobs, int flags)
{
    ...
    if (updCols) {
        rc = osql_send_updcols(osql->target_host, osql->rqid, osql->uuid,
...
int osql_serial_send_readset(struct sqlclntstate *clnt, int nettype)
{
    ...
    rc = osql_send_serial(osql->target_host, osql->rqid, osql->uuid, arr_ptr,
...
static int osql_sock_restart(struct sqlclntstate *clnt, int maxretries, int keep_session, int is_final)
{
    ...
    do {
        ...
        osql->clientinfo_sent = 0;  /* and re-send clientinfo */
        if (osql->tablename) {
            free(osql->tablename);
            osql->tablename = NULL;
            osql->tablenamelen = 0;
...
/* Send OSQL_CLIENTINFO once per transaction. Default OFF for the same reason. */
int gbl_osql_send_clientinfo = 0;
                                     ...
                                     int nettype)
{
    ...
    /* Once per transaction, ahead of the usedb dedup. The taskname is the only
     * part the master cannot already see -- it has the host from the session. */
    if (gbl_osql_send_clientinfo && !osql->clientinfo_sent && clnt->argv0) {
        do {
            rc = osql_send_clientinfo(osql->target_host, osql->rqid, osql->uuid, clnt->argv0, clnt->last_pid, nettype);
            RESTART_SOCKSQL;
        } while (restarted);
        if (rc)
            return rc;
        osql->clientinfo_sent = 1;
        osql->replicant_numops++;
        DEBUG_PRINT_NUMOPS();
    }
    ...
    if (gbl_osql_send_fingerprint && !fingerprint_is_zero(clnt->work.aFingerprint) &&
        ...
        do {
            rc = osql_send_fingerprint(osql->target_host, osql->rqid, osql->uuid, clnt->work.aFingerprint, nettype);
            RESTART_SOCKSQL;
        } while (restarted);
        if (rc)
            return rc;
    ...
    do {
        rc = osql_send_usedb(osql->target_host, osql->rqid, osql->uuid, tablename,
                                  ...
                                  struct sqlclntstate *clnt, int nettype)
{
    ...
    for (i = 0; i < pCur->db->nix; i++) {
        ...
        if (gbl_partial_indexes && pCur->db->ix_partial &&
            ...
            continue;
        rc = osql_send_index(osql->target_host, osql->rqid, osql->uuid, pCur->genid,
    ...
    for (int i = 0; i < pCur->db->schema->numblobs; i++) {
        ...
        if (updCols && gbl_osql_blob_optimization && blobs[i].length > 0) {
            ...
            if (idx >= 0 && idx < ncols && -1 == updCols[idx + 1]) {
                ...
                rc = osql_send_qblob(osql->target_host, osql->rqid, osql->uuid, i,
        ...
        rc = osql_send_qblob(osql->target_host, osql->rqid, osql->uuid,
    ...
    do {
        ...
        if (clnt->use_2pc && clnt->dist_txnid) {
            ...
            if (clnt->is_coordinator) {
                ...
                rc = osql_send_dist_txnid(osql->target_host, osql->rqid, osql->uuid, clnt->dist_txnid, clnt->dist_timestamp,
                                          nettype);

                for (p = clnt->participants.top; rc == 0 && p != NULL; p = p->linkv.next) {
                    rc = osql_send_participant(osql->target_host, osql->rqid, osql->uuid, p->participant_name,
        ...
        if (rc == 0 && gbl_osql_send_startgen && clnt->start_gen > 0) {
            ...
            if (osql->rqid == 0) {
                ...
                logmsg(LOGMSG_ERROR,
                       ...
                       "target_host=%s uuid=%s start_gen=%u dbtran.mode=%d nettype=%d\n",
                       __func__, retries, osql->sock_started,
                       osql->target_host ? osql->target_host : "(null)",
        ...
        if (rc == 0) {
            osql->replicant_numops++;
            rc = osql_send_commit(osql->target_host, osql->uuid, osql->replicant_numops,
...
int osql_begin_participant(struct sql_thread *thd)
{
    ...
    do {
        rc = osql_send_prepare(osql->target_host, osql->rqid, osql->uuid, clnt->dist_txnid, clnt->coordinator_dbname,
...
int osql_query_dbglog(struct sql_thread *thd, int queryid)
{
    ...
    do {
        rc = osql_send_dbglog(osql->target_host, osql->rqid, osql->uuid, new_cookie,
                                       ...
                                       unsigned long long genid, int nettype)
{
    ...
    do {
        ...
        if (rc == SQLITE_OK) {
            ...
            rc = osql_send_recordgenid(osql->target_host, osql->rqid, osql->uuid,
                                       genid, nettype);
            if (gbl_master_swing_sock_restart_sleep) {
                     ...
                     genid_t genid)
{
    ...
    if (rc != SQLITE_OK)
        return rc;
    return osql_send_dbq_consume(osql->target_host, osql->rqid, osql->uuid, genid,
                                 NET_OSQL_SOCK_RPL);
}
...
int osql_schemachange_logic(struct schema_change_type *sc, int usedb)
{
    ...
    if (thd->clnt->dbtran.mode == TRANLEVEL_SOSQL) {
        ...
        do {
            rc = osql_send_schemachange(osql->target_host, osql->rqid,
...
int osql_bpfunc_logic(struct sql_thread *thd, BpfuncArg *arg)
{
    ...
    if (thd->clnt->dbtran.mode == TRANLEVEL_SOSQL) {
        START_SOCKSQL;
        do {
            rc = osql_send_bpfunc(osql->target_host, osql->rqid,
...
int osql_send_del_qdb_logic(struct sqlclntstate *clnt, char *tablename, genid_t id)
{
    ...
    return osql_send_delrec(osql->target_host, osql->rqid, osql->uuid, id, -1,
                            NET_OSQL_SOCK_RPL);
}
