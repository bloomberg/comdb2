...
static void sync_capture_blobs(BtCursor *dta, int rrn, unsigned long long genid)
{
    ...
    if (dta->ixnum >= 0 && !(db->schema && db->schema->ix && (db->schema->ix[dta->ixnum]->flags & SCHEMA_DATACOPY)))
        return;
...
/* Called when one of the sorter's blocking windows opens: each list sort (the
   in-memory sort at rewind, and every spill to a PMA), and each merge step once
   the sort has spilled.  Leaves sort_probe_last_us alone; windows can be as
   short as one merge step, and resetting it per window would defeat the rate
   limit. */
...
/* Called from inside the sorter's comparison loops, every
   ...
   Releasing here is safe: the sorter walks its own records and touches no
   btree.  At rewind the read phase is already over.  A spill made during
   OP_SorterInsert comes mid-scan, but between two rows, with the scan's
   cursors at rest -- the state cursor_move_postop releases from anyway. */
        ...
        *pRes = 0;
        ...
        if (bias == OP_DeferredSeek && pCur->synced_at_generation &&
            ...
            /* Serving from the buffers still has to record the genid for
               ...
               table cursor.) */
            if (!gbl_selectv_rangechk && pCur->is_recording && clnt->ctrl_sqlengine == SQLENG_INTRANS_STATE) {
...
/* here, if we are a splinter, we update the key properly */
    ...
    if (verify && pCur->db && !pCur->bt->is_temporary && pCur->rootpage != RTPAGE_SQLITE_MASTER && *pRes != 0 &&
