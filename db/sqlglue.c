...
/* Cost of asking "is anyone waiting on my page locks?" (bdb_curtran_has_waiters
   ...
   not reach the lockwait_ buckets.  Yet it runs per cursor move and takes a
   locker-partition lock to read a couple of bytes, so on a scan-heavy workload
   the sheer call count makes it a real cost.  (LOCKREGION is compiled out --
   see berkdb/dbinc/lock.h -- so no global region mutex is involved, and the
   partition is keyed by lockerid, so sessions rarely collide.)
