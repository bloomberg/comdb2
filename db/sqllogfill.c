...
static void printstats(void)
{
    ...
    if (now - lastpr > 0) {
        ...
        logmsg(LOGMSG_USER,
               ...
               records_applied, bytes_applied, sql_finds, sql_nexts, enque_blocks, qsz);
        lastpr = now;
    }
}
...
static void request_logs_from_master(bdb_state_type *bdb_state)
{
    ...
    while (!db_is_exiting() && !bdb_lock_desired(bdb_state)) {
        ...
        rc = snprintf(sql_cmd, SQL_CMD_LEN, "select * from comdb2_transaction_logs('{%u:%u}', NULL, 9, %d)",
                      last_lsn.file, last_lsn.offset, timeout);
        ...
        while (!(desired = bdb_lock_desired(bdb_state)) && !(exiting = db_is_exiting()) &&
            ...
            /* Turns out this makes a huge difference .. */
    ...
    while (!db_is_exiting()) {
        ...
        if (bdb_lock_desired(bdb_state)) {
            ...
            while (bdb_lock_desired(bdb_state) && !db_is_exiting()) {
                if (gbl_debug_sql_logfill) {
                    logmsg(LOGMSG_USER, "%s: released bdb-lock, lock-desired, cnt=%d\n", __func__, cnt);
                }
                sleep(1);
            }
            BDB_READLOCK(__func__);
