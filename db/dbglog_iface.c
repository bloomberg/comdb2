...
struct comdb2buf *open_dbglog_file(unsigned long long cookie)
{
    if (impl.open_dbglog_file)
        return impl.open_dbglog_file(cookie);
    return 0;
}
void append_debug_logs_from_master(struct comdb2buf *sb, unsigned long long cookie)
{
...
void dump_client_query_stats(struct comdb2buf *sb, struct client_query_stats *stats)
{
    if (impl.dump_client_query_stats)
        impl.dump_client_query_stats(sb, stats);
}

void dump_client_query_stats_packed(struct comdb2buf *sb, const uint8_t *pakd)
{
