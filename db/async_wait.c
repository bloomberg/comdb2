        ...
        if (log_compare(&(item->seqnum.lsn), &(add_before_lsn->seqnum.lsn)) < 0) {
            if (gbl_async_dist_commit_verbose) {
                logmsg(LOGMSG_DEBUG, "thread %p Adding item %d:%d to lsn_list before %d:%d\n", (void *)pthread_self(),
                       item->seqnum.lsn.file, item->seqnum.lsn.offset, add_before_lsn->seqnum.lsn.file,
                       add_before_lsn->seqnum.lsn.offset);
            }
...
void add_to_absolute_ts_list(async_wait_node *item)
{
    ...
    if (gbl_async_dist_commit_verbose) {
        logmsg(LOGMSG_DEBUG, "Adding item %d:%d to absolute_ts_list with next time stamp %ld\n", item->seqnum.lsn.file,
               item->seqnum.lsn.offset, item->next_ts);
    }
...
int free_work_item(async_wait_node *item)
{
    ...
    time_metric_add(thedb->async_wait_wait_for_first_ack_time,
                    item->wait_wait_for_first_ack_end_time - item->wait_wait_for_first_ack_start_time);
    time_metric_add(thedb->async_wait_wait_for_all_ack_time,
                    item->wait_wait_for_all_ack_end_time - item->wait_wait_for_all_ack_start_time);
    time_metric_add(thedb->async_wait_wait_finish_time,
                    item->wait_wait_finish_end_time - item->wait_wait_finish_start_time);
    time_metric_add(thedb->async_wait_wait_next_commit_timestamp_time,
                    item->wait_wait_next_commit_timestamp_end_time - item->wait_wait_next_commit_timestamp_start_time);
    time_metric_add(thedb->async_wait_wait_notify_time,
                    item->wait_wait_notify_end_time - item->wait_wait_notify_start_time);
...
void process_work_item(async_wait_node *item)
{
    ...
    case INIT:
        ...
        if (item->track_once && item->bdb_state->attr->track_replication_times) {
            ...
            if (do_slow_node_check && item->bdb_state->attr->track_replication_times &&
    ...
    case WAIT_FOR_FIRST_ACK:
        ...
        if (((comdb2_time_epochms() - item->start_time) < item->bdb_state->attr->rep_timeout_maxms) &&
            ...
            for (int i = 0; i < item->numnodes; i++) {
                ...
                if (rc == 0) {
                    ...
                    if (item->waitms < item->bdb_state->attr->rep_timeout_minms) {
                        /* If the first node responded really fast, we don't want to impose too harsh a timeout on the
                         * remaining nodes */
                        item->waitms = item->bdb_state->attr->rep_timeout_minms;
                    }
                    if (item->bdb_state->rep_trace) {
                        logmsg(LOGMSG_USER,
                               "fastest node to <%d:%d> was %dms, will wait "
            ...
            if (!lock_desired)
                ...
                logmsg(LOGMSG_WARN, "lock desired, not waiting for initial replication of <%d:%d>\n",
                       item->seqnum.lsn.file, item->seqnum.lsn.offset);
        }
        item->wait_wait_for_first_ack_end_time = comdb2_time_epochms();
    case WAIT_FOR_ALL_ACK:
    got_first_ack_label:
        if (item->cur_state != WAIT_FOR_ALL_ACK) {
            item->cur_state = WAIT_FOR_ALL_ACK;
            item->wait_wait_for_all_ack_start_time = comdb2_time_epochms();
            item->is_final_check = 0;
        }
        if (item->waitms >= 0 || !item->is_final_check) {
            item->numfailed = 0;
            // int acked = 0;
            ...
            if (item->waitms <= 0) {
                if (gbl_async_dist_commit_verbose) {
                    logmsg(LOGMSG_USER, "This is the final pass over replicants\n");
                }
                item->is_final_check = 1;
            }
            for (int i = 0; i < item->numnodes; i++) {
                ...
                if (rc != 0 && rc != -2 && (rc == -999 && item->is_final_check)) {
                    ...
                    if (nodegen <= item->seqnum.generation && log_compare(&(item->seqnum.lsn), &nodelsn) >= 0) {
                        ...
                        if (item->bdb_state->attr->catchup_on_commit && item->bdb_state->attr->catchup_window) {
                            DB_LSN *masterlsn =
                                &(item->bdb_state->seqnum_info->seqnums[nodeix(item->bdb_state->repinfo->master_host)]
                                      .lsn);
                            int cntbytes = subtract_lsn(item->bdb_state, masterlsn, &nodelsn);
                            set_coherent_state(item->bdb_state, item->nodelist[i],
                                               (cntbytes < item->bdb_state->attr->catchup_window)
                                                   ? STATE_INCOHERENT_WAIT
                                                   : STATE_INCOHERENT,
                                               __func__, __LINE__);

                        } else {
                            set_coherent_state(item->bdb_state, item->nodelist[i], STATE_INCOHERENT, __func__,
                                               __LINE__);
                        }
                        /* change next_commit_timestamp for the work queue, if new value of coherency_commit_timestamp
                           is larger, than current value of coherency_commit_timestamp */
        ...
        if (item->numfailed == 0) {
            if (gbl_async_dist_commit_verbose) {
                logmsg(LOGMSG_USER, "All replicants have caught up!\n");
            }
        } else if (item->waitms <= 0 && !item->is_final_check) {
            if (gbl_async_dist_commit_verbose) {
                logmsg(LOGMSG_USER, "ran out of waitms.. Doing final pass\n");
            }
            goto got_first_ack_label;
    ...
    wait_finish_label:
        ...
        if (!item->numfailed && !item->numskip && !item->numwait &&
            item->bdb_state->attr->remove_commitdelay_on_coherent_cluster && item->bdb_state->attr->commitdelay) {
        ...
        if (item->durable_lsns) {
            item->outrc =
                is_txn_durable(item->bdb_state, item->num_commissioned, item->num_successfully_acked, &(item->seqnum));
        }
    ...
    case WAIT_NEXT_COMMIT_TIMESTAMP:
        ...
        if (bdb_attr_get(item->iq->dbenv->bdb_attr, BDB_ATTR_COHERENCY_LEASE)) {
            ...
            if (next_commit > now) {
                ...
                if (gbl_async_dist_commit_verbose) {
                    logmsg(LOGMSG_USER,
                           "now: %ld, next_commit_timestamp: %ld."
    ...
    wait_notify_label:
        ...
        if (item->outrc == 0 && item->iq->sorese->rcout == 0 && item->errstat.errval == COMDB2_SCHEMACHANGE_OK) {
        ...
        if (wait_rc == 0) {
            ...
            while (item != NULL) {
                if (log_compare(&item->seqnum.lsn, &local_max_lsn_so_far) > 0) {
                    ...
                    if (gbl_async_dist_commit_verbose) {
                        logmsg(LOGMSG_USER,
                               "Current item LSN %d:%d larger than max_so_far %d:%d. Stop processing LSN list\n",
                               item->seqnum.lsn.file, item->seqnum.lsn.offset, local_max_lsn_so_far.file,
                               local_max_lsn_so_far.offset);
                    }
