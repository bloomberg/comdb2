...
struct seqnum_wait {
     ...
     * block processor returns.  target_host is interned (never freed), so the
     * bare pointer stays valid after the osql session is torn down. */
    const char *target_host;
    unsigned long long rqid;
    uuid_t uuid;
    /* snap_info is owned by the osql session, which is torn down as soon as
     * the block processor returns -- long before we signal.  Keep our own
     * copy; have_snap distinguishes "no snap_info" from an all-zero one. */
    snap_uid_t snap;
    int have_snap;
    int nops;
    int rcout;
 ...
 * must wait inline (queue full, or not initialized). */
