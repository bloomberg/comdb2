    ...
    */
    ...
    /* we cant actually BECOME a different version of comdb2 at runtime,
       ...
       version of comdb2 than we are */
        ...
        bdb_attr_set(dbenv->bdb_attr, BDB_ATTR_SNAPISOL, 1);
        gbl_snapisol = 1;
        if (!gbl_do_not_use_modsnap_for_snapshot) {
            gbl_use_modsnap_for_snapshot = 1;
            gbl_modsnap_asof = 1;
            logmsg(LOGMSG_INFO, "Enabled snapshot isolation (default modsnap)\n");

            // These should be off, but just to make sure.
            gbl_new_snapisol = 0;
            gbl_new_snapisol_asof = 0;
            gbl_new_snapisol_logging = 0;
        }
        ...
        // Do not turn off gbl_snapisol
        gbl_use_modsnap_for_snapshot = 0;
        gbl_do_not_use_modsnap_for_snapshot = 1;
        gbl_modsnap_asof = 0;

        // if new snapisol becomes the fallback implementation:
        //      if gbl_snapisol -> turn on new_snapisol tunables
        // as of right now the fallback is the oldest snapshot implementation
