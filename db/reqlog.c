...
static void reqlog_free_stmt(struct reqlogger *logger) {
    if (logger->stmt) {
        free(logger->stmt);
        logger->stmt = NULL;
    }
...
static void print_ym(const intv_t *tv, char *s, int n)
{
    ...
    snprintf(s, n, "%s%u-%2.2u", tv->sign == -1 ? "-" : "", ym->years,
             ym->months);
}
...
static int current_long_request_count;
static int current_long_request_duration_ms;
static int current_longest_long_request_ms;
static int current_shortest_long_request_ms;
...
static void reqlog_log_longreq(struct sqlclntstate *clnt)
{
    ...
    if (clnt->sql)
        reqlog_set_sql(&logger, clnt->sql);
                        ...
                        int line)
{
    ...
    if (logger->durationus >= M2U(long_request_thresh)) {
        ...
        Pthread_mutex_lock(&reqlog_longreq_log_mtx);
        {
            ...
            if (logger->durationus < M2U(shortest_long_request_ms)) {
                ...
                if (long_request_out != default_out) {
                    ...
                    if (sqlinfo) {
                        if (long_request_count == 1) {
                            logmsg(LOGMSG_USER,
                                   "LONG REQUEST (%d ms) finished [%s] (see %s for details)\n",
                            ...
                            logmsg(LOGMSG_USER,
                                   ...
                                   longest_long_request_ms, sqlinfo,
                                   long_request_out->filename);
                        }
                        free(sqlinfo);
                    } else {
                        if (long_request_count == 1) {
                            logmsg(LOGMSG_USER,
                                   "LONG REQUEST (%d ms) finished (see %s for details)\n",
                                   U2M(logger->durationus),
                                   long_request_out->filename);
                        } else {
                            logmsg(LOGMSG_USER,
            ...
            while ((nclntstats = hash_get_num_entries(clientstats) + 1) >
                ...
                if (old_entry) {
                    ...
                    if (old_entry->rawtotals.fingerprints) {
                        hash_for(old_entry->rawtotals.fingerprints,
                                 hash_free_element, NULL);
...
static int print_client_fingerprint(struct reqlogger *logger, char *host,
                                    char *task, char *fp, int64_t count,
                                    int64_t cost, int64_t time, int64_t rows)
{
    reqlog_logf(logger, REQL_INFO,
                "host=%s task=%s fp=%s count=%" PRId64 " cost=%" PRId64
                " time=%" PRId64 " rows=%" PRId64 "\n",
...
{
    struct dump_client_sql_options *options =
        (struct dump_client_sql_options *)arg;
    struct query_count *cnt = (struct query_count *)ent;
    char fingerprint[FINGERPRINTSZ * 2 + 1];

    util_tohex(fingerprint, cnt->fingerprint, FINGERPRINTSZ);
    if (options->do_snap) {
        if (cnt->count != cnt->last_count || cnt->cost != cnt->last_cost ||
            cnt->timems != cnt->last_timems || cnt->rows != cnt->last_rows) {
            print_client_fingerprint(
                options->logger, options->st->host, options->st->task,
                fingerprint, cnt->count - cnt->last_count,
                cnt->cost - cnt->last_cost, cnt->timems - cnt->last_timems,
        ...
        print_client_fingerprint(options->logger, options->st->host,
                                 options->st->task, fingerprint, cnt->count,
                                 cnt->cost, cnt->timems, cnt->rows);
    }
    return 0;
}
...
{
    struct dump_client_sql_options options = {.do_snap = do_snapshot,
                                              .logger = logger};
    Pthread_rwlock_wrlock(&clientstats_lk);
    if (clientstats)
                                 ...
                                 int timems)
{
    ...
    if (stats->fingerprints == NULL) {
        ...
        stats->fingerprints = hash_init_o(
            offsetof(struct query_count, fingerprint), FINGERPRINTSZ);
        if (stats->fingerprints == NULL)
            abort();
    }
...
{
    struct client_sql_systable_options *opt =
        (struct client_sql_systable_options *)arg;
    struct query_count *cnt = (struct query_count *)ent;

    if (opt->nstats == opt->nalloced) {
        opt->nalloced = opt->nalloced * 2 + 16;
        void *p = realloc(opt->stats, sizeof(struct client_sql_systable_data) *
                                          opt->nalloced);
        if (p == NULL)
            return 1;
        opt->stats = p;
...
int gather_client_sql_data_single(void *ent, void *arg)
{
    ...
    int rc = 0;
    if (st->rawtotals.fingerprints)
        rc = hash_for(st->rawtotals.fingerprints,
                      gather_client_sql_data_fingerprint, arg);
    Pthread_mutex_unlock(&st->rawtotals.lk);
    return rc;
}
...
{
    struct client_sql_systable_data *stats =
        (struct client_sql_systable_data *)data;
