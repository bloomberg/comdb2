                         ...
                         int dixnum, int nonewrefs, int flags)
{
    ...
    strncpy(bwdct->tablename, srcdb->tablename_ip, MAXTABLELEN);
    bwdct->dstdb = dstdb;
    bwdct->blkpos = blkpos;
    bwdct->sixlen = keylen;
    bwdct->sixnum = sixnum;
                             ...
                             int *errout)
{
    ...
    for (i = 0; i < iq->usedb->n_rev_constraints; i++) {
        ...
        for (j = 0; j < cnstrt->nrules; j++) {
            ...
            /*
            rc=stag_to_stag_buf(iq->usedb->tablename_ip, ".ONDISK",rec_dta,
            ...
            if (newrec_dta != NULL) {
                /*
                rc=stag_to_stag_buf(iq->usedb->tablename_ip, ".ONDISK",newrec_dta,
...
int verify_del_constraints(struct ireq *iq, void *trans, int *errout)
{
    ...
    while (rc == 0) {
        ...
        if (bct->nonewrefs) {
            if (iq->debug)
                reqprintf(iq,
                          ...
                          bct->dstdb->tablename_ip, bct->dixnum, bct->tablename, bct->sixnum);
            reqmoref(iq, " RC %d", rc);
        ...
        keylen = rc = stag_to_stag_buf_ckey(bct->tablename, ondisk_tag, skey, bct->dstdb->tablename_ip, dondisk_tag, dkey,
                                            &nullck, FK2PK);
        if (rc == -1) {
            if (iq->debug)
                reqprintf(iq,
    ...
    *blkpos = -1;
    ...
    do {
        ...
        for (idx = 0; idx < limit; idx++) {
            ...
            /*
            rc=stag_to_stag_buf(iq->usedb->tablename_ip, ".ONDISK",od_dta,
               ondisk_tag, key, NULL);
             */
            if (iq->idxInsert)
            ...
            for (cidx = 0; cidx < nct; cidx++) {
                ...
                for (ridx = 0; ridx < ct->nrules; ridx++) {
                    ...
                    /*snprintf(ondisk_tag, MAXTAGLEN, ".ONDISK_IX_%d", fixnum);
                      rc=stag_to_stag_bufx(iq->usedb->tablename_ip,
                      ".ONDISK",od_dta,
