...
int release_locks_on_emit_row(struct sqlclntstate *clnt)
{
    ...
    /* Short circuit if we don't have any waiters.  Timed on every Nth call --
       see the gbl_probe_us note in sqlglue.c.  Reached only with
       sql_release_locks_on_emit_row on, which is off by default; the per-row
       probe that is always live on the feature's path is in cursor_move_postop. */
...
void clnt_to_ruleset_item_criteria(
  ...
  clnt->authdata = clnt->plugin.get_authdata(clnt);
