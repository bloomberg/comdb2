...
void print_idle_clnt(struct sqlclntstate *clnt)
{
    ...
    ctrace("fd:%d  idle:%ldmins  origin:%s  [prev=> process:%s pid:%d sql:%s]  [process:%s pid:%d sql:%s] opcode:%s "
           "local-cache:%c\n",
           get_fileno(clnt), elapsed.tv_sec / 60, clnt->origin, clnt->prev_argv0, clnt->prev_pid, clnt->second_last_sql,
           clnt->argv0, clnt->conninfo.pid, clnt->last_sql, clnt->prev_opcode, clnt->in_local_cache ? 'Y' : 'N');
}
