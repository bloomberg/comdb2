...
static int logical_cron_create(sched_if_t *intf,
                           ...
                           struct errstat *err)
{
    ...
    rc = (cron_add_event(sched, NULL, *nrolls, _logical_cron_test_callback,
                arg1, arg2, arg3, (uuid_t*)&source_id, err, NULL) == NULL)?
...
int logical_cron_unit_test(FILE *out, const char *name)
{
    ...
    if (logical_cron_create(&intf, NULL/*logical_describe*/, NULL/*logical_event_describe*/)) {
    ...
    sched = cron_add_event(NULL, name, 1, _logical_cron_test_callback, "testing",
                           &nrolls, &sched, &source_id, &err, &intf);
    if(!sched) {
