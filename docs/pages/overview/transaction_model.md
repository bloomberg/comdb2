...
2. Fail the transaction and have the replicant retry.  The failed transaction is fully rolled back.  The replicant then re-runs all the statements that were part of the transaction from scratch against the current state of the data, generates a new list of operations to execute, and re-submits it to the master.

Neither solution is ideal.  The first places an additional burden on application developers for the case where the application doesn't care that the row no longer exists.  The second may change the outcome of the statement since the data it reads may have changed (consider replaying ```UPDATE employee SET salary = salary + 100``` — the retry reads the current salary, which now reflects the other transaction's commit, producing a different result than the first attempt would have).  Having to choose between 2 evils, the default is option (2).  Applications can choose option (1) by running the SQL statement ```SET VERIFYRETRY OFF```.
