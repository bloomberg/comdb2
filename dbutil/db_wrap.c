...
uint32_t dbfile_is_sparse(dbfile_info *f)
{
    ...
    if (statbuf.st_size > (statbuf.st_blocks * statbuf.st_blksize)) return 1;
...
int read_write_file(dbfile_info *f, void *writer_ctx, writer w)
{
    int fd;
    int rc;
    ...
    while (bytes_left > 0) {
        ...
        rc = w(writer_ctx, pagebuf, bytes_read, current_file_offset);
        if (rc < 0) {
            return 1;
        }
