...
int is_valid_table_name(const char *const name, size_t len,
  ...
  if (len == 0) {
    if (error) {
      *error = comdb2_asprintf("Table name cannot be empty", len, name);
    }
    return 1;
  }
  ...
  if (len > MAXTABLELEN - 1) {
    if (error) {
      *error =
          comdb2_asprintf("Table name is too long. max_length=%d length=%zu",
  ...
  if (check_for_illegal_chars &&
    ...
    if (error) {
      *error = comdb2_asprintf("Table name contains illegal characters", name);
    }
    return 3;
  }
