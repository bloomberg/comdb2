...
size_t schemachange_packed_size(struct schema_change_type *s)
{
    ...
    s->packed_len =
        sizeof(s->kind) + sizeof(s->not_used) + sizeof(s->uuid) +
...
void *buf_put_schemachange(struct schema_change_type *s, void *p_buf, void *p_buf_end)
{
    ...
    p_buf = buf_put(&s->not_used, sizeof(s->not_used), p_buf, p_buf_end);
                              ...
                              void *p_buf_end)
{
    ...
    p_buf = (uint8_t *)buf_get(&s->not_used, sizeof(s->not_used), p_buf, p_buf_end);
