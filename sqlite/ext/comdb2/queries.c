...
sqlite3_module systblQueriesModule = {
    .access_flag = CDB2_ALLOW_USER,
};

typedef struct systable_queries {
    char *fingerprint;
...
int queries_systable_collect(void **data, int *nrecords)
{
    *nrecords = 0;
    *data = NULL;
    void *ent;
    ...
    *nrecords = hash_get_num_entries(gbl_sample_queries_hash);
    ...
    int idx = 0;
    systable_queries_t *arr = calloc(*nrecords, sizeof(systable_queries_t));
...
void queries_systable_free(void *arr, int nrecords)
{
    systable_queries_t *parr = (systable_queries_t *)arr;
    int i;
...
int systblQueriesInit(sqlite3*db)
{
    return create_system_table(
        db, "comdb2_queries", &systblQueriesModule,
        queries_systable_collect, queries_systable_free,
        sizeof(systable_queries_t),
        CDB2_CSTRING, "fingerprint", -1, offsetof(systable_queries_t, fingerprint),
        CDB2_CSTRING, "plan_fingerprint", -1, offsetof(systable_queries_t, plan_fingerprint),
        CDB2_CSTRING, "query", -1, offsetof(systable_queries_t, query),
        CDB2_CSTRING | SYSTABLE_FIELD_NULLABLE, "query_plan", -1, offsetof(systable_queries_t, query_plan),
        CDB2_CSTRING | SYSTABLE_FIELD_NULLABLE, "params", -1, offsetof(systable_queries_t, params),
        CDB2_DATETIME, "timestamp", -1, offsetof(systable_queries_t, timestamp),
