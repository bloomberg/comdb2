 ...
 * 3. Plan contains any other constraint on 'filename', or any constraint on 'type'
 ...
 * accesses. They are just meant to roughly differentiate different plans.
*/
...
int files_util_best_index(sqlite3_vtab *tab, sqlite3_index_info *pIdxInfo)
{
    ...
    logmsg(LOGMSG_DEBUG,
           "%s: Estimated cost for index with %d constraints is %f\n", __func__, pIdxInfo->nConstraint, estimated_cost);
