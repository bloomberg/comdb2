...
/* Grow packedprev/packed to at least 'need' bytes and return it; NULL on
 * failure or if 'need' exceeds MAXBLOBLENGTH2.  Freed in logicalopsClose().
 * Plain realloc()/free(): sqlite's allocator caps a single allocation well
 * below an odh2 record's ~2GB ceiling. */
        ...
        bdb_osql_log_rec_t *rec, DBT *logdta)
{
     ...
     * rather than a fixed 256MB, so an odh2 record is not truncated. */
