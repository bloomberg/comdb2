...
{
    if (isTemp || isView || isVirtual || pParse->db->init.busy || IN_DECLARE_VTAB || use_sqlite_impl(pParse)) {
        pParse->comdb2_ddl_ctx = 0;
    ...
    )
{
    ...
    if (temp || use_sqlite_impl(pParse) || pParse->db->init.busy || IN_DECLARE_VTAB) {
