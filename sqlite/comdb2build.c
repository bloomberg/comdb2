...
out:
    ...
    Table *pTab;
    char *zName, *zSrcName;
    char *zDb = "main"; /*locals*/
    BpfuncArg *arg = NULL;
    sqlite3 *db = pParse->db;
    Vdbe *v  = sqlite3GetVdbe(pParse);

    if (comdb2AuthenticateUserOp(v, pParse)) {
        setError(pParse, SQLITE_INTERNAL, "Authentication error");
        goto exit_rename_table;
    }

    zSrcName = sqlite3NameFromToken(db, pSrcName);
    if( !zSrcName ) {
        setError(pParse, SQLITE_INTERNAL, "Oom Error");
        goto exit_rename_table;
    }

    if (gbl_allow_user_schema) {
        char tblName[MAXTABLELEN];
        struct SrcList_item src = {0};
        src.zName = zSrcName;
        resolveTableName(&src, zDb, tblName);
        pTab = sqlite3LocateTable(pParse, 0, tblName, zDb);
    } else {
        pTab = sqlite3LocateTable(pParse, 0, zSrcName, zDb);
    }
    if( !pTab ) {
        setError(pParse, SQLITE_INTERNAL, "Table not existing");
        goto exit_rename_table;
    }
    ...
    arg->type = BPFUNC_TABLE_RENAME;
    arg->table_rename = malloc(sizeof(BpfuncTableRename));
    if(!arg->table_rename) {
        setError(pParse, SQLITE_INTERNAL, "Oom Error");
        goto exit_rename_table;
    }

    bpfunc_table_rename__init(arg->table_rename);
    arg->table_rename->src_tablename = zSrcName;
    arg->table_rename->dst_tablename = zName;
