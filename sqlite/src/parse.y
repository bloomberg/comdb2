...
partition_options ::= TIME PERIOD STRING(P) RETENTION INTEGER(R) START STRING(S). {
  comdb2CreateTimePartition(pParse, &P, &R, &S, 0);
}
partition_options ::= TIME PERIOD STRING(P) RETENTION INTEGER(R) START STRING(S) RETROACTIVELY. {
  comdb2CreateTimePartition(pParse, &P, &R, &S, 1);
}
...
%ifdef SQLITE_BUILDING_FOR_COMDB2
  ...
  REBUILD READ READONLY REC RESERVED RESUME RETENTION RETROACTIVELY REVOKE RLE ROWLOCKS
