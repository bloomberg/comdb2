...
void comdb2AlterRetention(Parse *pParse, Token *retention, int is_manual)
{
    ...
    /* validate the target here too so the client gets a clear message; the
     * master revalidates with the same rules while holding the schema lock
     * (apply-time errors reach the client only generically) */
    {
        struct comdb2_ddl_context *ctx = pParse->comdb2_ddl_ctx;
        const char *part = ctx ? ctx->tablename : "";
        struct errstat err = {0};

        if (!timepart_is_partition(part)) {
            setError(pParse, SQLITE_ERROR, "Not a time partition");
            free_ddl_context(pParse);
            return;
        }
        if (timepart_validate_retention_change(part, ret, is_manual, &err)) {
            setError(pParse, SQLITE_ERROR, err.errstr);
            free_ddl_context(pParse);
            return;
        }
    }
