                                  ...
                                  int check_shard, int *table_exists)
{
    ...
    if(gbl_allow_user_schema && thd->clnt->current_user.name[0] != '\0' &&
       strcasecmp(thd->clnt->current_user.name, DEFAULT_USER) != 0) {
...
static int comdb2AuthenticateUserDDL(const char *tablename)
{
     ...
     if (thd->clnt && tablename)
     {
        if (bdb_tbl_op_access_get(bdb_state, NULL, 0, 
            tablename, thd->clnt->current_user.name, &bdberr))
          return SQLITE_AUTH;
        else
            return SQLITE_OK;
     }
...
static int authenticateSC(const char * table,  Parse *pParse)
{
    ...
    struct sql_thread *thd = pthread_getspecific(query_info_key);
    if (username && strcmp(username+1, thd->clnt->current_user.name) == 0) {
        return 0;
...
void comdb2setPassword(Parse* pParse, Token* pwd, Token* nm)
{
    ...
    if (comdb2AuthenticateUserDDL(""))
    {
        ...
        if (!(thd && thd->clnt &&
              strcmp(thd->clnt->current_user.name, password->user) == 0 )) {
            setError(pParse, SQLITE_AUTH, "User does not have OP credentials");
            goto clean_arg;
        }
    }
                      ...
                      size_t len)
{
   ...
   } else if (thd->clnt && (thd->clnt->current_user.name[0] != '\0') &&
              !strchr(p->zName, '@') &&
