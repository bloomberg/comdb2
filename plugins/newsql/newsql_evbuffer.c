...
static void newsql_read_again_hdr(int fd, short what, void *arg)
{
    struct newsql_appdata_evbuffer *appdata = arg;
    add_lru_evbuffer(&appdata->clnt);
    read_newsql_hdr(-1, 0, appdata);
}
...
static void newsql_read_again(int fd, short what, void *arg)
{
    ...
    event_once(appsock_rd_base, newsql_read_again_hdr, appdata);
}

static void newsql_reset_evbuffer(struct newsql_appdata_evbuffer *appdata)
{
    if (appdata->first_run == 2) {
        appdata->first_run = 1;
    }
...
static void newsql_first_read(int fd, short what, void *arg)
{
    newsql_read_again_hdr(fd, what, arg);
}

static void newsql_setup_clnt_evbuffer(struct evbuffer *rd_buf, int fd, struct sockaddr_in *addr, int admin)
{
    int local = 0;
    ...
    event_once(appsock_rd_base, newsql_first_read, appdata);
}

static void handle_newsql_request_evbuffer(struct evbuffer *rd_buf, int fd, struct sockaddr_in *addr)
{
    newsql_setup_clnt_evbuffer(rd_buf, fd, addr, 0);
}

static void handle_newsql_admin_request_evbuffer(struct evbuffer *rd_buf, int fd, struct sockaddr_in *addr)
{
    newsql_setup_clnt_evbuffer(rd_buf, fd, addr, 1);
}
