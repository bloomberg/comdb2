...
static void newsql_setup_clnt_evbuffer(struct evbuffer *rd_buf, int fd, struct sockaddr_in *addr, int admin)
{
    ...
    appdata->rd_hdr_ev = event_new(appsock_rd_base, fd, EV_READ, rd_hdr, appdata);
    appdata->rd_payload_ev = event_new(appsock_rd_base, fd, EV_READ, rd_payload, appdata);
    appdata->writer = sqlwriter_new(&appdata->clnt, fd, newsql_pack, newsql_pack_hb, sizeof(struct newsqlheader));
    event_once(appsock_rd_base, newsql_read_hdr, appdata);
}
