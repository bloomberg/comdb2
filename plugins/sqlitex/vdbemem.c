...
** this:    assert( sqlitexVdbeCheckMemInvariants(pMem) );
*/
int sqlitexVdbeCheckMemInvariants(Mem *p){
  ...
  ** That saves a few cycles in inner loops. */
  ...
  assert( p->szMalloc==0
       || p->szMalloc==sqlitexDbMallocSize(p->db,p->zMalloc) );
...
** between formats.
*/
int sqlitexVdbeChangeEncoding(Mem *pMem, int desiredEnc){
  ...
  assert( pMem->db==0 || sqlitex_mutex_held(pMem->db->mutex) );
  ...
  */
  rc = sqlitexVdbeMemTranslate(pMem, (u8)desiredEnc);
...
*/
SQLITE_NOINLINE int sqlitexVdbeMemGrow(Mem *pMem, int n, int bPreserve){
  assert( sqlitexVdbeCheckMemInvariants(pMem) );
  assert( (pMem->flags&MEM_RowSet)==0 );
  ...
  ** contain a valid string or blob value.  */
  ...
  assert( pMem->szMalloc==0
       || pMem->szMalloc==sqlitexDbMallocSize(pMem->db, pMem->zMalloc) );
  if( pMem->szMalloc<n ){
    if( n<32 ) n = 32;
    if( bPreserve && pMem->szMalloc>0 && pMem->z==pMem->zMalloc ){
      pMem->z = pMem->zMalloc = sqlitexDbReallocOrFree(pMem->db, pMem->z, n);
      bPreserve = 0;
    }else{
      if( pMem->szMalloc>0 ) sqlitexDbFree(pMem->db, pMem->zMalloc);
      pMem->zMalloc = sqlitexDbMallocRaw(pMem->db, n);
    }
    if( pMem->zMalloc==0 ){
...
*/
int sqlitexVdbeMemClearAndResize(Mem *pMem, int szNew){
  assert( szNew>0 );
  assert( (pMem->flags & MEM_Dyn)==0 || pMem->szMalloc==0 );
  if( pMem->szMalloc<szNew ){
    return sqlitexVdbeMemGrow(pMem, szNew, 0);
  }
...
*/
int sqlitexVdbeMemMakeWriteable(Mem *pMem){
  int f;
  assert( pMem->db==0 || sqlitex_mutex_held(pMem->db->mutex) );
  assert( (pMem->flags&MEM_RowSet)==0 );
  ExpandBlob(pMem);
  ...
  if( (f&(MEM_Str|MEM_Blob)) && (pMem->szMalloc==0 || pMem->z!=pMem->zMalloc) ){
    if( sqlitexVdbeMemGrow(pMem, pMem->n + 2, 1) ){
      return SQLITE_NOMEM;
    }
    pMem->z[pMem->n] = 0;
    pMem->z[pMem->n+1] = 0;
...
int sqlitexVdbeMemExpandBlob(Mem *pMem){
  if( pMem->flags & MEM_Zero ){
    ...
    if( sqlitexVdbeMemGrow(pMem, nByte, 1) ){
      return SQLITE_NOMEM;
    }
...
*/
int sqlitexVdbeMemNulTerminate(Mem *pMem){
  assert( pMem->db==0 || sqlitex_mutex_held(pMem->db->mutex) );
...
** are converted using sqlitex_snprintf().  Converting a BLOB to a string
** is a no-op.
**
...
** sqlitex_value_text()), or for ensuring that values to be used as btree
** keys are strings. In the former case a NULL pointer is returned the
** user and the latter is an internal programming error.
*/
int sqlitexVdbeMemStringify(Mem *pMem, u8 enc, u8 bForce){
  int rc = SQLITE_OK;
  int fg = pMem->flags;
  const int nByte = 32;
  ...
  if( sqlitexVdbeMemClearAndResize(pMem, nByte) ){
    return SQLITE_NOMEM;
  }

  /* For a Real or Integer, use sqlitex_snprintf() to produce the UTF-8
  ** string representation of the value. Then, if the required encoding
  ** is UTF-16le or UTF-16be do a translation.
  ** 
  ** FIX ME: It would be better if sqlitex_snprintf() could do UTF-16.
  */
  /* COMDB2 MODIFICATION */
  ...
  if( fg & MEM_Interval ){
    ...
    else if (pMem->du.tv.type == INTV_DECIMAL_TYPE)
    {
       ...
       if(sqlitexDecimalToString( &pMem->du.tv.u.dec, tmp, sizeof(tmp)))
       {
    ...
    {
       fprintf(stderr, "BUG %s:%d type=%d\n", __FILE__, __LINE__, pMem->du.tv.type);
       return SQLITE_INTERNAL;
    }

    if (pMem->zMalloc) {
        sqlitexDbFree(pMem->db, pMem->zMalloc);
        pMem->zMalloc = NULL;
        pMem->szMalloc = 0;
        pMem->z = NULL;
    }
    ...
    pMem->n = strlen(tmp);
    z = sqlitexGlobalConfig.m.xMalloc(pMem->n+2);
    ...
    sqlitexVdbeChangeEncoding(pMem, enc);
    ...
    if( convMem2ClientDatetimeStrX(pMem, tmp, sizeof(tmp), &outdtsz)) { 
        char *z;
        sqlitexErrorWithMsg(pMem->db, SQLITE_CONV_ERROR, "can't convert datetime value to string");
  ...
  if (0);
    ...
    if( fg & MEM_Int ){
      sqlitex_snprintf(nByte, pMem->z, "%lld", pMem->u.i);
    }else{
      assert( fg & MEM_Real );
      sqlitex_snprintf(nByte, pMem->z, "%!.15g", pMem->u.r);
    }
...
** otherwise.
*/
int sqlitexVdbeMemFinalize(Mem *pMem, FuncDef *pFunc){
  int rc = SQLITE_OK;
...
**
** This is a helper routine for sqlitexVdbeMemSetNull() and
** for sqlitexVdbeMemRelease().  Use those other routines as the
** entry point for releasing Mem resources.
*/
static SQLITE_NOINLINE void vdbeMemClearExternAndSetNull(Mem *p){
  assert( p->db==0 || sqlitex_mutex_held(p->db->mutex) );
  assert( VdbeMemDynamic(p) );
  if( p->flags&MEM_Agg ){
    sqlitexVdbeMemFinalize(p, p->u.pDef);
  ...
  if( p->flags&MEM_Dyn && p->xDel ){ /* COMDB2 MODIFICATION */
    ...
    sqlitexRowSetClear(p->u.pRowSet);
...
**
** This is a helper routine invoked by sqlitexVdbeMemRelease() in
...
**
** Use sqlitexVdbeMemSetNull() to release just the Mem.xDel space
...
*/
i64 sqlitexVdbeIntValue(Mem *pMem){
  int flags;
  assert( pMem->db==0 || sqlitex_mutex_held(pMem->db->mutex) );
  assert( EIGHT_BYTE_ALIGNMENT(pMem) );
  flags = pMem->flags;
  ...
  else if( pMem->flags & MEM_Interval)
  {
    ...
    {
       fprintf(stderr, "%s:%d: Cannot convert a decimal to integer\n", __FILE__, __LINE__);
       return 0;
    }
  }
  else if( pMem->flags & MEM_Datetime)
  {
    ...
    sqlitexAtoi64(pMem->z, &value, pMem->n, pMem->enc);
    return value;
  }else{
    return 0;
  }
...
*/
double sqlitexVdbeRealValue(Mem *pMem){
  assert( pMem->db==0 || sqlitex_mutex_held(pMem->db->mutex) );
  assert( EIGHT_BYTE_ALIGNMENT(pMem) );
  ...
  else if( pMem->flags & MEM_Interval)
  {
    ...
    double val = (double)0;
    sqlitexAtoF(pMem->z, &val, pMem->n, pMem->enc);
    return val;
  }else{
    /* (double)0 In case of SQLITE_OMIT_FLOATING_POINT... */
    return (double)0;
...
** MEM_Int if we can.
*/
void sqlitexVdbeIntegerAffinity(Mem *pMem){
  i64 ix;
...
*/
int sqlitexVdbeMemIntegerify(Mem *pMem){
  assert( pMem->db==0 || sqlitex_mutex_held(pMem->db->mutex) );
  assert( (pMem->flags & MEM_RowSet)==0 );
  assert( EIGHT_BYTE_ALIGNMENT(pMem) );

  pMem->u.i = sqlitexVdbeIntValue(pMem);
  MemSetTypeFlag(pMem, MEM_Int);
  return SQLITE_OK;
}
...
*/
int sqlitexVdbeMemRealify(Mem *pMem){
  assert( pMem->db==0 || sqlitex_mutex_held(pMem->db->mutex) );
  assert( EIGHT_BYTE_ALIGNMENT(pMem) );

  pMem->u.r = sqlitexVdbeRealValue(pMem);
...
*/
int sqlitexVdbeMemNumerify(Mem *pMem){
  if( (pMem->flags & (MEM_Int|MEM_Real|MEM_Null))==0 ){
    assert( (pMem->flags & (MEM_Blob|MEM_Str|MEM_Interval|MEM_Datetime))!=0 );
    assert( pMem->db==0 || sqlitex_mutex_held(pMem->db->mutex) );
  /* TODO: COMDB2 MODIFICATION */
  /* string is NULL for cast(intervalds at string) */
    if( pMem->z && 0==sqlitexAtoi64(pMem->z, &pMem->u.i, pMem->n, pMem->enc) ){
      MemSetTypeFlag(pMem, MEM_Int);
    }else{
      pMem->u.r = sqlitexVdbeRealValue(pMem);
      MemSetTypeFlag(pMem, MEM_Real);
      sqlitexVdbeIntegerAffinity(pMem);
    }
  }
...
*/
int sqlitexVdbeMemCast(Vdbe *p, Mem *pMem, u8 aff, u8 encoding){
  ...
  switch( aff ){
    case SQLITE_AFF_NONE: {   /* Really a cast to BLOB */
      if( (pMem->flags & MEM_Blob)==0 ){
        sqlitexValueApplyAffinity(pMem, SQLITE_AFF_TEXT, encoding);
    ...
    case SQLITE_AFF_NUMERIC: {
      sqlitexVdbeMemNumerify(pMem);
      break;
    }
    case SQLITE_AFF_INTEGER: {
      sqlitexVdbeMemIntegerify(pMem);
      break;
    }
    case SQLITE_AFF_REAL:
    /* COMDB2 MODIFICATION */
    case SQLITE_AFF_SMALL: {
      sqlitexVdbeMemRealify(pMem);
      break;
    }
    case SQLITE_AFF_DATETIME: {
      rc2 = sqlitexVdbeMemDatetimefyTz(pMem, pMem->tz);
      if (rc2 && !paulbitchk(368, 18)) {
        sqlitexSetString(&p->zErrMsg, db, "cast to datetime failed");
    ...
    case SQLITE_AFF_INTV_SE: {
      rc2 = sqlitexVdbeMemIntervalfy(pMem, aff);
      if (rc2 && !paulbitchk(368, 18)) {
        sqlitexSetString(&p->zErrMsg, db, "cast to interval failed");
        sqlite3SetConversionError();
        rc = rc2;
      }
      break;
    ...
    case SQLITE_AFF_DECIMAL: {
      rc2 = sqlitexVdbeMemDecimalfy(pMem);
      if (rc2 && !paulbitchk(368, 18)) {
        sqlitexSetString(&p->zErrMsg, db, "cast to decimal failed");
        sqlite3SetConversionError();
        rc = rc2;
      }
      break;
...
*/
void sqlitexVdbeMemInit(Mem *pMem, sqlitex *db, u16 flags){
...
** To free all resources, use sqlitexVdbeMemRelease(), which both calls this
** routine to invoke the destructor and deallocates Mem.zMalloc.
**
** Use this routine to reset the Mem prior to insert a new value.
**
** Use sqlitexVdbeMemRelease() to complete erase the Mem prior to abandoning it.
*/
void sqlitexVdbeMemSetNull(Mem *pMem){
...
void sqlitexValueSetNull(sqlitex_value *p){
  sqlitexVdbeMemSetNull((Mem*)p); 
}
...
*/
void sqlitexVdbeMemSetZeroBlob(Mem *pMem, int n){
  sqlitexVdbeMemRelease(pMem);
...
*/
void sqlitexVdbeMemSetInt64(Mem *pMem, i64 val){
  if( VdbeMemDynamic(pMem) ){
...
** manifest type REAL.
*/
void sqlitexVdbeMemSetDouble(Mem *pMem, double val){
  sqlitexVdbeMemSetNull(pMem);
  if( !sqlitexIsNaN(val) ){
    pMem->u.r = val;
    pMem->flags = MEM_Real;
...
** empty boolean index.
*/
void sqlitexVdbeMemSetRowSet(Mem *pMem){
  sqlitex *db = pMem->db;
  assert( db!=0 );
  assert( (pMem->flags & MEM_RowSet)==0 );
  sqlitexVdbeMemRelease(pMem);
  pMem->zMalloc = sqlitexDbMallocRaw(db, 64);
  if( db->mallocFailed ){
    pMem->flags = MEM_Null;
    pMem->szMalloc = 0;
  }else{
    assert( pMem->zMalloc );
    pMem->szMalloc = sqlitexDbMallocSize(db, pMem->zMalloc);
    pMem->u.pRowSet = sqlitexRowSetInit(db, pMem->zMalloc, pMem->szMalloc);
...
*/
int sqlitexVdbeMemTooBig(Mem *p){
  assert( p->db!=0 );
...
*/
void sqlitexVdbeMemAboutToChange(Vdbe *pVdbe, Mem *pMem){
  int i;
  Mem *pX;
...
*/
void sqlitexVdbeMemShallowCopy(Mem *pTo, const Mem *pFrom, int srcType){
...
*/
int sqlitexVdbeMemCopy(Mem *pTo, const Mem *pFrom){
  int rc = SQLITE_OK;
  ...
  if( pTo->flags&(MEM_Str|MEM_Blob) ){
    if( 0==(pFrom->flags&MEM_Static) ){
      pTo->flags |= MEM_Ephem;
      rc = sqlitexVdbeMemMakeWriteable(pTo);
    }
  }
...
*/
void sqlitexVdbeMemMove(Mem *pTo, Mem *pFrom){
  assert( pFrom->db==0 || sqlitex_mutex_held(pFrom->db->mutex) );
  assert( pTo->db==0 || sqlitex_mutex_held(pTo->db->mutex) );
  assert( pFrom->db==0 || pTo->db==0 || pFrom->db==pTo->db );

  sqlitexVdbeMemRelease(pTo);
...
** parameter. If the value passed is SQLITEX_TRANSIENT, then the 
...
int sqlitexVdbeMemSetStr(
  ...
  */
  if( xDel==SQLITEX_TRANSIENT ){
    ...
    testcase( nAlloc==31 );
    testcase( nAlloc==32 );
    if( sqlitexVdbeMemClearAndResize(pMem, MAX(nAlloc,32)) ){
      return SQLITE_NOMEM;
    }
    ...
    pMem->z = (char *)z;
    pMem->xDel = xDel;
    flags |= ((xDel==SQLITEX_STATIC)?MEM_Static:MEM_Dyn);
  }
...
#ifndef SQLITE_OMIT_UTF16
  if( pMem->enc!=SQLITE_UTF8 && sqlitexVdbeMemHandleBom(pMem) ){
    return SQLITE_NOMEM;
  }
#endif
...
*/
int sqlitexVdbeMemSetDecimal(Mem *pMem, decQuad *val){
  sqlitexVdbeMemSetNull(pMem);
...
int sqlitexVdbeMemFromBtree(
  ...
  ** that both the BtShared and database handle mutexes are held. */
  ...
  if( offset+amt<=available ){
    ...
    pMem->flags = MEM_Null;
    if( SQLITE_OK==(rc = sqlitexVdbeMemClearAndResize(pMem, amt+2)) ){
      if( key ){
...
*/
static SQLITE_NOINLINE const void *valueToText(sqlitex_value* pVal, u8 enc){
  assert( pVal!=0 );
  assert( pVal->db==0 || sqlitex_mutex_held(pVal->db->mutex) );
  ...
  if( pVal->flags & (MEM_Blob|MEM_Str) ){
    ...
    if( pVal->flags & MEM_Zero ){
      sqlitexVdbeMemExpandBlob(pVal);
    }
    if( pVal->enc != (enc & ~SQLITE_UTF16_ALIGNED) ){
      sqlitexVdbeChangeEncoding(pVal, enc & ~SQLITE_UTF16_ALIGNED);
    }
    if( (enc & SQLITE_UTF16_ALIGNED)!=0 && 1==(1&SQLITE_PTR_TO_INT(pVal->z)) ){
      assert( (pVal->flags & (MEM_Ephem|MEM_Static))!=0 );
      if( sqlitexVdbeMemMakeWriteable(pVal)!=SQLITE_OK ){
        return 0;
      }
    }
    sqlitexVdbeMemNulTerminate(pVal); /* IMP: R-31275-44060 */
  }else{
    /* COMDB2 MODIFICATION */
    if (sqlitexVdbeMemStringify(pVal, enc, 0) != SQLITE_OK)
      return 0;
...
** external API. It works in a similar way to sqlitex_value_text(),
...
** boundary.
*/
const void *sqlitexValueText(sqlitex_value* pVal, u8 enc){
  if( !pVal ) return 0;
  assert( pVal->db==0 || sqlitex_mutex_held(pVal->db->mutex) );
  assert( (enc&3)==(enc&~SQLITE_UTF16_ALIGNED) );
  assert( (pVal->flags & MEM_RowSet)==0 );
...
/*
** Create a new sqlitex_value object.
*/
sqlitex_value *sqlitexValueNew(sqlitex *db){
  Mem *p = sqlitexDbMallocZero(db, sizeof(*p));
  if( p ){
    p->flags = MEM_Null;
    p->db = db;
  }
...
/*
** Context object passed by sqlitexStat4ProbeSetValue() through to 
...
/*
** Allocate and return a pointer to a new sqlitex_value object. If
** the second argument to this function is NULL, the object is allocated
** by calling sqlitexValueNew().
**
** Otherwise, if the second argument is non-zero, then this function is 
** being called indirectly by sqlitexStat4ProbeSetValue(). If it has not
** already been allocated, allocate the UnpackedRecord structure that 
** that function will return to its caller here. Then return a pointer to
** an sqlitex_value within the UnpackedRecord.a[] array.
*/
static sqlitex_value *valueNew(sqlitex *db, struct ValueNewStat4Ctx *p){
  ...
  if( p ){
    ...
    if( pRec==0 ){
      ...
      pRec = (UnpackedRecord*)sqlitexDbMallocZero(db, nByte);
      if( pRec ){
        pRec->pKeyInfo = sqlitexKeyInfoOfIndex(p->pParse, pIdx);
        if( pRec->pKeyInfo ){
...
** If the result is a text value, the sqlitex_value object uses encoding 
** enc.
**
...
static int valueFromFunction(
  ...
  sqlitex_value **ppVal,          /* Write the new value here */
  struct ValueNewStat4Ctx *pCtx   /* Second argument for valueNew() */
){
  sqlitex_context ctx;            /* Context object for function invocation */
  sqlitex_value **apVal = 0;      /* Function arguments */
  int nVal = 0;                   /* Size of apVal[] array */
  FuncDef *pFunc = 0;             /* Function definition */
  sqlitex_value *pVal = 0;        /* New value */
  int rc = SQLITE_OK;             /* Return code */
  ...
  nName = sqlitexStrlen30(p->u.zToken);
  pFunc = sqlitexFindFunction(db, p->u.zToken, nName, nVal, enc, 0);
  assert( pFunc );
  ...
  if( pList ){
    apVal = (sqlitex_value**)sqlitexDbMallocZero(db, sizeof(apVal[0]) * nVal);
    if( apVal==0 ){
      rc = SQLITE_NOMEM;
      goto value_from_function_out;
    }
    for(i=0; i<nVal; i++){
      rc = sqlitexValueFromExpr(db, pList->a[i].pExpr, enc, aff, &apVal[i]);
  ...
  if( ctx.isError ){
    rc = ctx.isError;
    sqlitexErrorMsg(pCtx->pParse, "%s", sqlitex_value_text(pVal));
  }else{
    sqlitexValueApplyAffinity(pVal, aff, SQLITE_UTF8);
    assert( rc==SQLITE_OK );
    rc = sqlitexVdbeChangeEncoding(pVal, enc);
    if( rc==SQLITE_OK && sqlitexVdbeMemTooBig(pVal) ){
      rc = SQLITE_TOOBIG;
      pCtx->pParse->nErr++;
    }
  }
 ...
 value_from_function_out:
  ...
  if( apVal ){
    for(i=0; i<nVal; i++){
      sqlitexValueFree(apVal[i]);
    }
    sqlitexDbFree(db, apVal);
  }
...
** above sqlitexValueFromExpr(). Allocate the sqlitex_value object
** using valueNew().
**
** If pCtx is NULL and an error occurs after the sqlitex_value object
...
static int valueFromExpr(
  ...
  ** into valueFromFunction(). */
  ...
  if( op==TK_CAST ){
    u8 aff = sqlitexAffinityType(pExpr->u.zToken,0);
    ...
    if( *ppVal ){
      ...
      sqlitexVdbeMemCast(p, *ppVal, aff, SQLITE_UTF8);
      sqlitexValueApplyAffinity(*ppVal, affinity, SQLITE_UTF8);
    }
    return rc;
  }
  ...
  */
  ...
  if( op==TK_STRING || op==TK_FLOAT || op==TK_INTEGER ){
    ...
    if( ExprHasProperty(pExpr, EP_IntValue) ){
      sqlitexVdbeMemSetInt64(pVal, (i64)pExpr->u.iValue*negInt);
    }else{
      zVal = sqlitexMPrintf(db, "%s%s", zNeg, pExpr->u.zToken);
      if( zVal==0 ) goto no_mem;
      sqlitexValueSetStr(pVal, -1, zVal, SQLITE_UTF8, SQLITE_DYNAMIC);
    }
    if( (op==TK_INTEGER || op==TK_FLOAT ) && affinity==SQLITE_AFF_NONE ){
      sqlitexValueApplyAffinity(pVal, SQLITE_AFF_NUMERIC, SQLITE_UTF8);
    }else{
      sqlitexValueApplyAffinity(pVal, affinity, SQLITE_UTF8);
    }
    ...
    if( SQLITE_OK==valueFromExpr(db,pExpr->pLeft,enc,affinity,&pVal,pCtx) 
     && pVal!=0
      ...
      sqlitexValueApplyAffinity(pVal, affinity, enc);
    }
  }else if( op==TK_NULL ){
    pVal = valueNew(db, pCtx);
    if( pVal==0 ) goto no_mem;
    ...
    nVal = sqlitexStrlen30(zVal)-1;
    assert( zVal[nVal]=='\'' );
    sqlitexVdbeMemSetStr(pVal, sqlitexHexToBlob(db, zVal, nVal), nVal/2,
                         0, SQLITE_DYNAMIC);
  }
#endif
...
no_mem:
  db->mallocFailed = 1;
  sqlitexDbFree(db, zVal);
  assert( *ppVal==0 );
#ifdef SQLITE_ENABLE_STAT3_OR_STAT4
  if( pCtx==0 ) sqlitexValueFree(pVal);
#else
  assert( pCtx==0 ); sqlitexValueFree(pVal);
#endif
  return SQLITE_NOMEM;
}

/*
** Create a new sqlitex_value object, containing the value of pExpr.
**
** This only works for very simple expressions that consist of one constant
** token (i.e. "5", "5.1", "'a string'"). If the expression can
** be converted directly into a value, then the value is allocated and
** a pointer written to *ppVal. The caller is responsible for deallocating
** the value by passing it to sqlitexValueFree() later on. If the expression
** cannot be converted to a value, then *ppVal is set to NULL.
*/
int sqlitexValueFromExpr(
  sqlitex *db,              /* The database connection */
  Expr *pExpr,              /* The expression to evaluate */
  u8 enc,                   /* Encoding to use */
  u8 affinity,              /* Affinity to use */
...
static void recordFunc(
  ...
  iSerial = sqlitexVdbeSerialType(argv[0], file_format);
  nSerial = sqlitexVarintLen(iSerial);
  nVal = sqlitexVdbeSerialTypeLen(iSerial);
  db = sqlitex_context_db_handle(context);

  nRet = 1 + nSerial + nVal;
  aRet = sqlitexDbMallocRaw(db, nRet);
  if( aRet==0 ){
    sqlitex_result_error_nomem(context);
  }else{
    aRet[0] = nSerial+1;
    putVarint32(&aRet[1], iSerial);
    sqlitexVdbeSerialPut(&aRet[1+nSerial], argv[0], iSerial);
    sqlitex_result_blob(context, aRet, nRet, SQLITEX_TRANSIENT);
    sqlitexDbFree(db, aRet);
  }
}
...
** Returns 0 if cast should have occured. Result of cast in *rc.
** Returns 1 for other affinity types or if string literal did not convert.
*/
static int castExpr(
  sqlitex *db,
  Expr *pExpr,
  ...
  switch( affinity ){
    case SQLITE_AFF_DATETIME:
      if( str_to_dttz(z, strlen(z), v->tzname, &dt, v->dtprec)==0 ){
        sqlitex_value *pVal = valueNew(db, pAlloc);
        if ( pVal==NULL ){
          return SQLITE_NOMEM;
        }
        pVal->du.dt = dt;
...
static int stat4ValueFromExpr(
  ...
  sqlitex_value **ppVal           /* OUT: New value object (or NULL) */
){
  int rc = SQLITE_OK;
  sqlitex_value *pVal = 0;
  ...
  if( !pExpr ){
    ...
    if( pVal ){
      sqlitexVdbeMemSetNull((Mem*)pVal);
    }
    ...
    sqlitexVdbeSetVarmask(pParse->pVdbe, iBindVar);
    if( (v = pParse->pReprepare)!=0 ){
      pVal = valueNew(db, pAlloc);
      if( pVal ){
        rc = sqlitexVdbeMemCopy((Mem*)pVal, &v->aVar[iBindVar-1]);
        if( rc==SQLITE_OK ){
          sqlitexValueApplyAffinity(pVal, affinity, ENC(db));
        }
        pVal->db = pParse->db;
      }
    }
...
**  * The expression is a bound variable, and this is a reprepare, or
**
**  * The sqlitexValueFromExpr() function is able to extract a value 
...
** as described for sqlitexStat4ProbeSetValue() above. 
**
...
** sqlitex_value object is allocated.
**
** If *ppVal is initially NULL then the caller is responsible for 
** ensuring that the value written into *ppVal is eventually freed.
*/
int sqlitexStat4Column(
  sqlitex *db,                    /* Database handle */
  const void *pRec,               /* Pointer to buffer containing record */
  int nRec,                       /* Size of buffer pRec in bytes */
  int iCol,                       /* Column to extract */
  sqlitex_value **ppVal           /* OUT: Extracted value */
){
  ...
  for(i=0; i<=iCol; i++){
    ...
    szField = sqlitexVdbeSerialTypeLen(t);
    iField += szField;
  }
  ...
  if( pMem==0 ){
    pMem = *ppVal = sqlitexValueNew(db);
    if( pMem==0 ) return SQLITE_NOMEM;
  }
  sqlitexVdbeSerialGet(&a[iField-szField], t, pMem);
  /* COMDB2 MODIFICATION */
  if(11 == t) /* see sqlitexVdbeSerialGet, 11 is datetime blob */
...
** by an earlier call to sqlitexStat4ProbeSetValue(). This call deletes
** the object.
*/
void sqlitexStat4ProbeFree(UnpackedRecord *pRec){
  if( pRec ){
    int i;
    int nCol = pRec->pKeyInfo->nField+pRec->pKeyInfo->nXField;
    Mem *aMem = pRec->aMem;
    sqlitex *db = aMem[0].db;
    for(i=0; i<nCol; i++){
      sqlitexVdbeMemRelease(&aMem[i]);
    }
    sqlitexKeyInfoUnref(pRec->pKeyInfo);
    sqlitexDbFree(db, pRec);
  }
}
...
/*
** Change the string value of an sqlitex_value object
*/
void sqlitexValueSetStr(
  sqlitex_value *v,     /* Value to be set */
  int n,                /* Length of string z */
  const void *z,        /* Text of the new string */
  u8 enc,               /* Encoding to use */
  void (*xDel)(void*)   /* Destructor for the string */
){
  if( v ) sqlitexVdbeMemSetStr((Mem *)v, z, n, enc, xDel);
}

/*
** Free an sqlitex_value object
*/
void sqlitexValueFree(sqlitex_value *v){
  if( !v ) return;
  sqlitexVdbeMemRelease((Mem *)v);
  sqlitexDbFree(((Mem*)v)->db, v);
}

/*
** Return the number of bytes in the sqlitex_value object assuming
** that it uses the encoding "enc"
*/
int sqlitexValueBytes(sqlitex_value *pVal, u8 enc){
  Mem *p = (Mem*)pVal;
  if( (p->flags & MEM_Blob)!=0 || sqlitexValueText(pVal, enc) ){
    if( p->flags & MEM_Zero ){
...
*/
int sqlitexVdbeMemDatetimefyTz(Mem *pMem, const char *tz){
  int fg = pMem->flags;
  if(fg & MEM_Null)
    return SQLITE_OK;
  if(fg & MEM_Interval)
...
** Add MEM_Datetime to the set of representations for the given Mem. 
*/
int sqlitexVdbeMemDatetimefy(Mem *pMem){
   return sqlitexVdbeMemDatetimefyTz(pMem, NULL);
}

/*
** Add MEM_Datetime to the set of representations for the given Mem. 
*/
int sqlitexVdbeMemDecimalfy(Mem *pMem)
{
   decContext     dfp_ctx;
   int            rc = SQLITE_OK;
   int            fg = pMem->flags;
   ...
   else if(fg & MEM_Str)
   {
      ...
      /* COMDB2 MODIFICATION. */
      sqlitexVdbeMemNulTerminate(pMem);
...
*/
int sqlitexVdbeMemIntervalAndInterval(const Mem *a, const Mem *b, int opcode, Mem * res) {
...
*/
int sqlitexVdbeMemIntervalAndInt(const Mem *a, const Mem *b, int opcode, Mem * res) {
    bzero(res, sizeof(Mem));
    res->flags |= MEM_Interval;
    long long aa = 0, bb = 0;
...
**  Operate on interval and int, res = a opcode b;
*/
int sqlitexVdbeMemIntAndInterval(const Mem *a, const Mem *b, int opcode, Mem * res) {
    bzero(res, sizeof(Mem));
    res->flags |= MEM_Interval;
    long long aa = 0, bb = 0;
...
*/
int sqlitexVdbeMemDatetimeAndDatetime(const Mem *a, const Mem *b, int opcode, Mem *res) {
  bzero(res, sizeof(Mem));
  res->flags |= MEM_Interval;
...
static int _dttz_to_native_datetime(cdb2_client_datetime_t * cdt, const Mem *inp) {
    ...
    if (!paulbitchk(368, 20) && (inp->du.dt.dttz_sec < -377705030401ll || inp->du.dt.dttz_sec > 253402214400ll ))
       return -1;
...
int sqlitexVdbeMemDatetimeAndInterval(const Mem *a, const Mem *b, int opcode, Mem * res) {
    ...
    if( b->du.tv.type == INTV_YM_TYPE) {
        if (a->du.dt.dttz_prec == DTTZ_PREC_MSEC) {
            ...
            /* convert server_datetimeus_t to cdb2_client_datetime_t */
            ...
            /* convert cdb2_client_datetime_t to server_datetimeus_t */
...
int sqlitexVdbeMemIntervalAndDatetime(const Mem *a, const Mem *b, int opcode, Mem * res) {
