...
int comdb2LocateSP(Parse *p, char *sp)
{
	...
	free(ver);
	if (rc0 < 0 && rc1 < 0) {
		sqlitexErrorMsg(p, "no such procedure: %s", sp);
		return -1;
	}
	return 0;
}
...
Cdb2TrigEvents *comdb2AddTriggerEvent(Parse *pParse, Cdb2TrigEvents *A, Cdb2TrigEvent *B)
{
	...
	default: sqlitexErrorMsg(pParse, "%s: bad op", __func__, B->op); return NULL;
	}
	if (B->op == e->op) {
		sqlitexDbFree(pParse->db, A);
		sqlitexErrorMsg(pParse, "%s condition repeated", type);
		return NULL;
	#ifndef ALLOW_ALL_COLS
	} else if (B->cols == NULL) {
		sqlitexDbFree(pParse->db, A);
		sqlitexErrorMsg(pParse, "%s condition has unspecified columns", type);
		return NULL;
	#endif
	}
	e->op = B->op;
...
{
	Table *table;
	if ((table = sqlitexLocateTableItem(parse, 0, &tbl->a[0])) == NULL) {
		sqlitexErrorMsg(parse, "no such table:%s", tbl->a[0].zName);
		return NULL;
	}
	Cdb2TrigTables *tmp;
	...
	if (tables) {
		...
		while (tmp) {
			if (strcmp(tmp->table->zName, name) == 0) {
				sqlitexErrorMsg(parse, "trigger already specified table:%s", name);
				return NULL;
			}
			tmp = tmp->next;
		}
	}
	tmp = sqlitexDbMallocRaw(parse->db, sizeof(Cdb2TrigTables));
	if (tmp == NULL) {
		sqlitexErrorMsg(parse, "malloc failED");
		return NULL;
	}
	tmp->table = table;
	tmp->events = events;
...
void comdb2CreateTrigger(Parse *parse, int dynamic, Token *proc, Cdb2TrigTables *tbl)
{
	...
	if (getqueuebyname(qname)) {
		sqlitexErrorMsg(parse, "trigger already exists: %s", spname);
		return;
	}
	...
	strbuf_free(s);
	Vdbe *v = sqlitexGetVdbe(parse);
	comdb2prepareNoRowsX(v, parse, 0, sc, &comdb2SqlSchemaChange,
...
void comdb2DropTrigger(Parse *parse, Token *proc)
{
	...
	Q4SP(qname, spname);
	if (!getqueuebyname(qname)) {
		sqlitexErrorMsg(parse, "no such trigger: %s", spname);
		return;
	}
...
#define comdb2DropFunc(parse, proc, pfx, type)                                 \
	do {                                                                   \
		...
		if (find_lua_##pfx##func(spname) == 0) {                       \
			sqlitexErrorMsg(parse, "no such lua " #type "func:%s", \
					spname);                               \
			return;                                                \
		}                                                              \
		struct schema_change_type *sc = new_schemachange_type();       \
