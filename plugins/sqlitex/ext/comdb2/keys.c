...
/* systbl_keys_cursor is a subclass of sqlitex_vtab_cursor which
...
struct systbl_keys_cursor {
  ...
  sqlitex_int64 iKeyid;      /* The column we're on */
};
...
#define STKEY_RECNUM    5
  ...
  rc = sqlitex_declare_vtab(db,
     "CREATE TABLE comdb2_keys(tablename,"
                                 "keyname,"
                                 "keynumber,"
                                 "isunique,"
                                 "isdatacopy,"
...
*/
static int systblKeysDisconnect(sqlitex_vtab *pVtab){
  sqlitex_free(pVtab);
  return SQLITE_OK;
}
...
*/
static int systblKeysOpen(sqlitex_vtab *p, sqlitex_vtab_cursor **ppCursor){
...
*/
static int systblKeysClose(sqlitex_vtab_cursor *cur){
  sqlitex_free(cur);
  return SQLITE_OK;
}
...
*/
static int systblKeysNext(sqlitex_vtab_cursor *cur){
...
static int systblKeysColumn(
  ...
  switch( i ){
    ...
    case STKEY_KEYNUM: {
      sqlitex_result_int64(ctx, (sqlitex_int64)pSchema->ixnum);
      break;
    }
    case STKEY_UNIQUE: {
      sqlitex_result_text(ctx, YESNO(!(pSchema->flags & SCHEMA_DUP)), -1, NULL);
      break;
    }
    case STKEY_DATACOPY: {
      sqlitex_result_text(ctx, YESNO(pSchema->flags & SCHEMA_DATACOPY), 
        -1, NULL);
      break;
    }
    case STKEY_RECNUM: {
      sqlitex_result_text(ctx, YESNO(pSchema->flags & SCHEMA_RECNUM), -1, NULL);
      break;
    }
  }
  return SQLITE_OK; 
...
** presently on.
*/
static int systblKeysRowid(sqlitex_vtab_cursor *cur, sqlite_int64 *pRowid){
  systbl_keys_cursor *pCur = (systbl_keys_cursor*)cur;
...
*/
static int systblKeysEof(sqlitex_vtab_cursor *cur){
  systbl_keys_cursor *pCur = (systbl_keys_cursor*)cur;
