...
static int systbl_connect(
  ...
  char **pErr
    ...
    strbuf_append(sql, ");");
    int rc = sqlitex_declare_vtab(db, strbuf_buf(sql));
    strbuf_free(sql);
    if (rc == SQLITE_OK) {
        ez_systable_vtab *vtab = calloc(1, sizeof(ez_systable_vtab));
        vtab->t = pAux;
        *ppVtab = (sqlitex_vtab*) vtab;
    }
    return rc;
}
...
static int systbl_disconnect(sqlitex_vtab *pVtab){
    free(pVtab);
  return SQLITE_OK;
}

static int systbl_open(sqlitex_vtab *p, sqlitex_vtab_cursor **ppCursor){
    ...
    *ppCursor = (sqlitex_vtab_cursor*) pCur;
    return rc;
}

static int systbl_close(sqlitex_vtab_cursor *cur){
...
static int systbl_next(sqlitex_vtab_cursor *cur){
    struct ez_systable_cursor *pCur = (struct ez_systable_cursor*) cur;
    pCur->rowid++;
    return SQLITE_OK;
}
  ...
  int i
    ...
    switch (t->fields[i].type & FIELD_TYPE_MASK) {
        case CDB2_INTEGER: {
            ...
            sqlitex_result_int64(ctx, *ival);
            break;
        }
        case CDB2_REAL: {
            double *dval = (double*) field;
            sqlitex_result_double(ctx, *dval);
            break;
        }
        case CDB2_CSTRING: {
            char *strval = *((char**)(rec + t->fields[i].offset));
            sqlitex_result_text(ctx, strval, -1, NULL);
            break;
        }
        case CDB2_BLOB: {
            systable_blobtype *blob = (systable_blobtype*) field;
            sqlitex_result_blob(ctx, blob->value, blob->size, NULL);
            break;
        }
        case CDB2_DATETIME:
        case CDB2_DATETIMEUS: {
            ...
            if ((t->fields[i].type & FIELD_TYPE_MASK) == CDB2_DATETIME) {
                ...
                rc = client_datetimeus_to_dttz((client_datetimeus_t*) dt, tz, &dtz, le);
            }

            if (rc == 0)
                sqlitex_result_datetime(ctx, &dtz, tz);
            break;
        }
		case CDB2_INTERVALYM: {
			intv_t interval;
            ...
            sqlitex_result_interval(ctx, &interval);
            break;
		}
...
static int systbl_rowid(sqlitex_vtab_cursor *cur, sqlite_int64 *pRowid){
    struct ez_systable_cursor *pCur = (struct ez_systable_cursor*) cur;
    *pRowid = pCur->rowid;
    return SQLITE_OK;
}
...
static int systbl_eof(sqlitex_vtab_cursor *cur){
    struct ez_systable_cursor *pCur = (struct ez_systable_cursor*) cur;
    return pCur->rowid >= pCur->npoints;
}

static const sqlitex_module systbl = {
...
void destroy_system_table(void *data) {
    struct systable *sys = data;
    free(sys->name);
    for (int i=0; i < sys->nfields; i++) {
        free(sys->fields[i].name);
    }
    free(sys->fields);
    free(sys);
}
...
int create_system_table(sqlitex *db, char *name, 
    ...
    int rc = sqlitex_create_module_v2(db, name, &systbl, sys, destroy_system_table);
    if (rc) {
        fprintf(stderr, "create rc %d %s\n", rc, sqlitex_errmsg(db));
        return rc;
    }
