...
static void get_sp_versions(systbl_sps_cursor *c) {
  ...
  c->pVer = sqlitex_malloc(sizeof(spversion) * (scnt + ccnt));
  int i;
  for(i = 0; i < scnt; ++i) {
    c->pVer[i].sVer = i + 1;
    c->pVer[i].cVer = NULL;
...
static void get_server_versioned_sps(char ***a, int *x) {
  ...
  while(1) {
    ...
    if (n == alloc) {
      alloc += inc;
      names = sqlitex_realloc(names, sizeof(char *) * alloc);
    }
...
#define STSP_SRC      4   /* source */
  ...
  rc = sqlitex_declare_vtab(db, "CREATE TABLE "
...
*/
static int systblSPsDisconnect(sqlitex_vtab *pVtab){
  sqlitex_free(pVtab);
  return SQLITE_OK;
}
...
static int systblSPsOpen(
  ...
  systbl_sps_cursor *pCur = sqlitex_malloc(sizeof(*pCur));
  ...
  *ppCursor = &pCur->base;
  ...
  sname = sqlitex_realloc(sname, sizeof(char *) * (ccnt + scnt));
  pCur->ppProc = sname;
  int i, j;
...
static int systblSPsClose(sqlitex_vtab_cursor *cur) {
  systbl_sps_cursor *pCur = (systbl_sps_cursor *)cur;
  for(int i = 0; i < pCur->nProcs; ++i) {
      free(pCur->ppProc[i]);
  }
  sqlitex_free(pCur->ppProc);
...
static int systblSPsNext(sqlitex_vtab_cursor *cur){
  systbl_sps_cursor *pCur = (systbl_sps_cursor*)cur;
  ++pCur->iVer;
...
*/
static int systblSPsEof(sqlitex_vtab_cursor *cur){
  systbl_sps_cursor *pCur = (systbl_sps_cursor*)cur;
...
static int systblSPsColumn(
  ...
  switch( col ) {
    case STSP_NAME:
      sp = c->ppProc[c->iProc];
      sqlitex_result_text( ctx, strdup(sp), -1, free );
      break;
    case STSP_VER:
      if (v == NULL)
        sqlitex_result_null( ctx );
      else if( v->cVer ) {
        sqlitex_result_text( ctx, strdup(v->cVer), -1, free );
        assert(v->sVer == 0);
      }
      else {
        char ver[32];
        sprintf( ver, "%d", v->sVer );
        sqlitex_result_text( ctx, strdup(ver), -1, free );
      }
      break;
    case STSP_CVER:
      if (v == NULL)
        ...
        sqlitex_result_text( ctx, YESNO( v->cVer ), -1, NULL );
      break;
    case STSP_DEF:
      if (v == NULL)
        sqlitex_result_null( ctx );
      ...
      sqlitex_result_text( ctx, YESNO( def ), -1, NULL );
      break;
    case STSP_SRC:
      sp = c->ppProc[c->iProc];
      if (v == NULL)
      ...
      if( src == NULL )
        ...
        sqlitex_result_text( ctx, src, size, free );
      break;
  }
  return SQLITE_OK;
};

static int systblSPsRowid(sqlitex_vtab_cursor *cur, sqlite_int64 *pRowid){
  systbl_sps_cursor *pCur = (systbl_sps_cursor*)cur;
  *pRowid = pCur->iProc;
  return SQLITE_OK;
}
