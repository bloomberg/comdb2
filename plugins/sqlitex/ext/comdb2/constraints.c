...
/* systbl_constraint_cursor is a subclass of sqlitex_vtab_cursor which
...
struct systbl_constraints_cursor {
  ...
  sqlitex_int64 iConstraintid; /* The rule we're on */
  sqlitex_int64 iRuleid;       /* The rule we're on */
};
...
*/
static int systblConstraintsDisconnect(sqlitex_vtab *pVtab){
  sqlitex_free(pVtab);
  return SQLITE_OK;
}
...
*/
static int systblConstraintsClose(sqlitex_vtab_cursor *cur){
  sqlitex_free(cur);
  return SQLITE_OK;
}
...
*/
static int systblConstraintsNext(sqlitex_vtab_cursor *cur){
...
static int systblConstraintsColumn(
  ...
  switch( i ){
    ...
    case STCON_KEY: {
      sqlitex_result_text(ctx, pConstraint->lclkeyname, -1, NULL);
      break;
    }
    case STCON_FTNAME: {
      sqlitex_result_text(ctx, pConstraint->table[pCur->iRuleid], -1, NULL);
      break;
    }
    case STCON_FKNAME: {
      sqlitex_result_text(ctx, pConstraint->keynm[pCur->iRuleid], -1, NULL);
      break;
    }
    case STCON_CDELETE: {
      sqlitex_result_text(ctx, YESNO(pConstraint->flags & CT_DEL_CASCADE), 
        -1, SQLITEX_STATIC);
      break;
    }
    case STCON_CUPDATE: {
      sqlitex_result_text(ctx, YESNO(pConstraint->flags & CT_UPD_CASCADE), 
        -1, SQLITEX_STATIC);
      break;
    }
  }
...
*/
static int systblConstraintsEof(sqlitex_vtab_cursor *cur){
  systbl_constraints_cursor *pCur = (systbl_constraints_cursor*)cur;
