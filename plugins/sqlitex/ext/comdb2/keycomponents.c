...
/* systbl_field_cursor is a subclass of sqlitex_vtab_cursor which
...
struct systbl_fields_cursor {
  ...
  sqlitex_int64 iRowid;      /* The tableid */
  sqlitex_int64 iKeyid;      /* The keyid within the table */
  sqlitex_int64 iFieldid;    /* The key field id within the key */
};
...
#define STFIELD_DEC        4
  ...
  rc = sqlitex_declare_vtab(db, "CREATE TABLE comdb2_keycomponents(tablename,"
                                "keyname,"
                                "columnnumber,"
                                "columnname,"
                                "isdescending)");
...
*/
static int systblFieldsDisconnect(sqlitex_vtab *pVtab){
  sqlitex_free(pVtab);
  return SQLITE_OK;
}
...
*/
static int systblFieldsOpen(sqlitex_vtab *p, sqlitex_vtab_cursor **ppCursor){
...
*/
static int systblFieldsClose(sqlitex_vtab_cursor *cur){
  sqlitex_free(cur);
  return SQLITE_OK;
}
...
*/
static int systblFieldsNext(sqlitex_vtab_cursor *cur){
...
static int systblFieldsColumn(
  ...
  switch( i ){
    ...
    case STFIELD_KEY: {
      sqlitex_result_text(ctx, pSchema->csctag, -1, NULL);
      break;
    }
    case STFIELD_COLNO: {
      sqlitex_result_int64(ctx, (sqlitex_int64)pCur->iFieldid);
      break;
    }
    ...
    case STFIELD_DEC: {
      sqlitex_result_text(ctx, YESNO(pField->flags & INDEX_DESCEND),
        -1, SQLITEX_STATIC);
      break;
    }
  }
...
*/
static int systblFieldsRowid(sqlitex_vtab_cursor *cur, sqlite_int64 *pRowid){
  systbl_fields_cursor *pCur = (systbl_fields_cursor*)cur;
  int i;
...
*/
static int systblFieldsEof(sqlitex_vtab_cursor *cur){
  systbl_fields_cursor *pCur = (systbl_fields_cursor*)cur;
