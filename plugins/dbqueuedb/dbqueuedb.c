...
static int get_stats(struct dbtable *, int, tran_type *, struct consumer_stat *);
...
static int get_stats(struct dbtable *db, int flags, tran_type *tran,
    ...
    if (db->odh) {
        ...
        rc = dbq_walk(&iq, flags, stat_callback, 0, (tran_type *)tran, st);
    }
    time_t epoch;
